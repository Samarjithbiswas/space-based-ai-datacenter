...
`orbital_dc.thermal.report_i_baseline()`.*

The work that opened this line of analysis asked a single, narrow question: can a node carrying several
high-power accelerators be held within its temperature limit at 650 kilometers using only passive
radiative cooling, with no refrigeration and no consumable coolant. The answer governs everything that
follows, because if the thermal case fails at the node level there is nothing to scale. This chapter
reproduces that original steady-state design point in full, since the rest of the book extends and
stress-tests it; the general thermal theory it draws on is developed in Chapter 10.

**The heat load.** The reference node dissipates four accelerators at a per-chip power of 300 watts,
together with avionics and a parasitic allowance:

$$Q_{\mathrm{total}} = 4 \times 300 + 150_{\mathrm{avionics}} + 100_{\mathrm{parasitic}} = 1450\ \mathrm{W}. \tag{2A.1}$$

**Radiator temperature.** All of this heat leaves as thermal radiation. For a gray emitter of area $A$
and emissivity $\varepsilon$ radiating from $n_s$ sides into a cold background, the steady-state panel
temperature follows from the Stefan-Boltzmann balance,
...
With two two-square-meter panels treated as a single-sided four-square-meter emitter, Z-93 white paint
at its end-of-life emissivity of 0.85, the panel settles at $T_{\mathrm{rad}} = 21.3\ ^\circ\mathrm{C}$.
The radiator is comfortable; the difficulty is entirely in getting the heat from the silicon to it.

**The conduction chain.** Between the transistor junction and the panel sits a series of thermal
resistances: junction to case, the thermal interface material, the cold-plate base, the heat pipe, and
the radiator interface. In series they add,

$$R_{\mathrm{th}} = r_{jc} + r_{\mathrm{tim}} + r_{\mathrm{base}} + r_{\mathrm{pipe}} + r_{\mathrm{rad}}. \tag{2A.3}$$

The original budget is $0.150 + 0.020 + 0.060 + 0.050 + 0.020 = 0.300\ \mathrm{K/W}$ after a modest
interface and heat-pipe improvement, down from $0.350\ \mathrm{K/W}$ before it. The junction-to-case
term dominates and is fixed by the package; the terms a designer can influence are the interface
material and the heat pipe.

**Junction temperature and the margin.** The junction sits above the radiator by the product of
per-chip power and chain resistance,
...
**Robustness.** A spacecraft thermal path carries redundant heat pipes, so the design must survive one
...
positive. The single-fault case survives.

**What the study did and did not establish.** Three caveats travel with this result, and they set the
agenda for the rest of the book. First, the per-chip power was taken as 300 watts; the actual figure for
the reference accelerator is undisclosed and is more plausibly in the 150-to-200-watt range, in which
case the junction sits well below the limit and the margin is larger, not smaller. The 300-watt case is
therefore conservative. Second, and pointing the other way, the analysis is a single-node steady state;
it does not by itself address the sharper wall that appears as per-chip power rises, where above roughly
400 watts the interface term alone overruns the entire budget and a passive chain can no longer cope, a
point developed in Chapter 10. Third, the original study was candid that the thermal case being feasible
does not make the system economical: its own break-even analysis was negative, which is exactly the
...
1. **Margin as a function of chain resistance.** Starting from the junction relation (2A.4) and the radiator balance (2A.2), show that the junction margin against the limit $T_{j,\mathrm{lim}}$ can be written as
$$\Delta T = T_{j,\mathrm{lim}} - \left(\frac{Q_{\mathrm{total}}}{\varepsilon\,\sigma\,A\,n_s}\right)^{1/4} - P_{\mathrm{chip}}\,R_{\mathrm{th}}.$$
Using this expression, determine the chain resistance $R_{\mathrm{th}}$ at which the margin vanishes, given the chapter's reference values $Q_{\mathrm{total}} = 1450\ \mathrm{W}$, $\varepsilon = 0.85$, $A\,n_s = 4\ \mathrm{m^2}$, $P_{\mathrm{chip}} = 300\ \mathrm{W}$, and $T_{j,\mathrm{lim}} = 125\ ^\circ\mathrm{C}$. State how much resistance headroom the baseline chain of (2A.3) retains.

2. **Sizing the radiator for a colder panel.** The closing argument of the chapter notes that radiator area is set by the fourth power of the chosen panel temperature. Using the sizing inversion (2A.9) with $Q_{\mathrm{total}} = 1450\ \mathrm{W}$ and $\varepsilon = 0.85$, compute the single-sided emitting area $A\,n_s$ required to settle the panel at $T_{\mathrm{rad}} = 0\ ^\circ\mathrm{C}$ rather than at the baseline $21.3\ ^\circ\mathrm{C}$. By what factor does the required area change relative to the baseline four square meters, and using (2A.4) by how many degrees does the junction margin improve if the chain resistance is held fixed at $0.300\ \mathrm{K/W}$.

3. **Quartic response of the radiator to load.** Using the cold-background balance (2A.2), show that for fixed $\varepsilon$, $A$, and $n_s$ the radiator temperature obeys $T_{\mathrm{rad}} \propto Q_{\mathrm{total}}^{1/4}$, and confirm the chapter's statement that doubling the load raises $T_{\mathrm{rad}}$ by the factor $2^{1/4}$. Then evaluate the absolute temperature rise of the baseline panel (starting from $T_{\mathrm{rad}} = 21.3\ ^\circ\mathrm{C} = 294.45\ \mathrm{K}$) if $Q_{\mathrm{total}}$ doubles to $2900\ \mathrm{W}$, and state the resulting reduction in junction margin using (2A.4) with $P_{\mathrm{chip}} = 300\ \mathrm{W}$ and $R_{\mathrm{th}} = 0.300\ \mathrm{K/W}$.

4. **Warm-sink sensitivity bound.** Equation (2A.8) gives the sensitivity of the radiator temperature to the sink temperature as $\partial T_{\mathrm{rad}}/\partial T_{\mathrm{sink}} = (T_{\mathrm{sink}}/T_{\mathrm{rad}})^3$. Taking the baseline panel temperature $T_{\mathrm{rad}} = 294.45\ \mathrm{K}$ as approximately fixed, use this sensitivity to estimate the first-order rise in $T_{\mathrm{rad}}$ produced by a sink at $T_{\mathrm{sink}} = 250\ \mathrm{K}$ acting over that interval, and discuss in light of the cold-background assumption behind (2A.2) whether the baseline margin of $13.7\ ^\circ\mathrm{C}$ from (2A.4) would survive such a sink. [expect a sensitivity of order $0.6$]
...
rather than prescribing fixed numbers, together with the quantitative allowances that flight
programs apply.

**Mass growth.** AIAA S-120-2006 sets a mass-growth allowance by hardware category for an estimated
design: 30 percent for small electronics and solar arrays, 25 percent for structures, mechanisms,
thermal control, batteries, and propulsion, 20 percent for large electronics, and 60 percent for
harness. On top of the per-item allowance, the NASA Goddard Integrated Design Center applies a
system-level mass margin of about 30 percent at the pre-Phase-A concept stage. Published mass-growth
data from operational programs show actual growth often exceeds the AIAA allowance, so these figures
are treated as lower bounds. The reference dry mass reported in Chapter 12 is a current-best-estimate;
applying the allowances above (implemented in the margins module) gives the predicted mass a program
would budget.

**Power.** The Goddard practice sizes the power system for a minimum 30 percent contingency of
delivered power over the current-best-estimate load, evaluated at the worst-case operational scenario
at end of life. The dawn-dusk power margin of Chapter 11 is interpreted against this rule.

**Readiness.** Progress is measured on the NASA nine-level technology-readiness scale, from basic
principles observed (level 1) to a system flight-proven through successful operations (level 9). The
components of the reference architecture sit near the middle: a single accelerator has operated in
...
| --- | --- | --- |
| Mass growth, small electronics and solar array | 30 percent | AIAA S-120-2006 |
| Mass growth, structures, thermal, battery, propulsion | 25 percent | AIAA S-120-2006 |
| Mass growth, large electronics | 20 percent | AIAA S-120-2006 |
| Mass growth, harness | 60 percent | AIAA S-120-2006 |
| System-level mass margin, pre-Phase-A | 30 percent | NASA GSFC Integrated Design Center |
| Power contingency over best estimate, worst-case end of life | 30 percent minimum | NASA GSFC Integrated Design Center |
...
**The two-body problem.** The circular relations above are special cases of the two-body problem.
Newton's law of gravitation and the second law give the relative acceleration of the satellite with
respect to Earth,

$$\ddot{\mathbf r} = -\frac{\mu}{r^{3}}\,\mathbf r, \qquad \mu = G(M_\oplus + m) \approx G M_\oplus, \tag{4.4}$$

where the approximation holds because the satellite mass is negligible against Earth's. Taking the
cross product of (4.4) with $\mathbf r$ shows that the specific angular momentum is conserved,

$$\mathbf h = \mathbf r \times \dot{\mathbf r}, \qquad \frac{d\mathbf h}{dt} = 0, \tag{4.5}$$

so the motion stays in a plane and the radius vector sweeps equal areas in equal times, which is
Kepler's second law.

**The orbit equation.** Integrating (4.4) once, using the conserved eccentricity (Laplace-Runge-Lenz)
vector, yields the trajectory as a conic section written in focal polar form,

$$r(\theta) = \frac{h^{2}/\mu}{1 + e\cos\theta} = \frac{p}{1 + e\cos\theta}, \qquad p = a\left(1 - e^{2}\right), \tag{4.6}$$

where $e$ is the eccentricity, $\theta$ the true anomaly measured from perigee, and $p$ the
semi-latus rectum. Bound orbits have $0 \le e < 1$; the circular orbit used throughout this study is
the special case $e = 0$, for which $r = a$ is constant and (4.1) to (4.3) follow directly.

**Kepler's third law.** For an ellipse of semi-axes $a$ and $b = a\sqrt{1-e^{2}}$, the constant
areal rate $\dot A = h/2$ integrated over the enclosed area $\pi a b$ gives the period

$$T = 2\pi\sqrt{\frac{a^{3}}{\mu}}, \tag{4.7}$$

independent of eccentricity, which is why the circular result (4.2) applies to the slightly
non-circular reference orbit without correction.

**Ground track and revisit.** Coverage planning needs the ground track, the locus of sub-satellite
points. While the satellite completes one orbit in time $T$, Earth turns beneath it at
...
1. Starting from the circular-speed relation (4.1) and the period relation (4.2), show that the orbital speed and period of a circular orbit are related by $v = 2\pi a / T$, and use this to derive Kepler's third law in the form $T^2 = (4\pi^2/\mu)\,a^3$. Then show that the specific energy (4.3) of a circular orbit can be written purely in terms of the speed as $\mathcal{E} = -v^2/2$, and explain why an energy loss therefore forces a decrease in $a$, consistent with the orbital-decay argument noted after (4.3).

2. Using the worked-example reference values ($\mu = 3.986\times10^{14}\ \mathrm{m^3/s^2}$, $a = 7.021\times10^6$ m) and the circular-speed and period relations (4.1) and (4.2), estimate the circular speed, the orbital period, and the number of orbits completed per day. Then, using $\Delta\lambda = \omega_\oplus T$ with $\omega_\oplus = 7.292\times10^{-5}$ rad/s, estimate the westward step of successive equator crossings. [expect $v \approx 7.5$ km/s, $T \approx 98$ min, near 15 orbits/day, $\Delta\lambda \approx 24.5^\circ$ per revolution]

3. The reference orbit lies at altitude $h = 650$ km. Consider raising the altitude to $h = 1200$ km, with $R_E$ taken as in the worked example so that $a = R_E + h$. Using the period relation (4.2), determine by what factor the orbital period changes between the two altitudes, and using (4.1) determine by what factor the circular speed changes. State whether the higher orbit completes more or fewer orbits per day.

4. A circular-orbit constellation is to be sized so that its ground track repeats after an integer number of nodal days. Using the repeat condition $k\,T = d\,(2\pi/\omega_\oplus)$ from the ground-track discussion together with the period relation (4.2), derive an expression for the semi-major axis $a$ that produces exactly $k$ orbits in $d$ nodal days, in terms of $\mu$, $\omega_\oplus$, $k$, and $d$. Then, using the reference values, evaluate $a$ for the case $k = 15$, $d = 1$ and compare the result to the reference orbit at $a = 7.021\times10^6$ m.
...
1. Starting from the secular disturbing function (5.3), show that Lagrange's planetary equation for the node (5.4) reduces to the precession rate (5.1) for a near-circular orbit. Carry out the derivative $\partial\bar R/\partial i$ explicitly, demonstrate that the resulting $\sin i$ in the numerator cancels the $\sin i$ in the denominator of (5.4), and confirm that the surviving dependence is $\cos i$ with the leading coefficient $-\tfrac{3}{2}\,n\,J_2\,(R_E/a)^2$. State clearly where the approximation $p = a(1-e^2) \approx a$ enters.

2. Using (5.6) with $\mu = 3.986\times10^{14}\ \mathrm{m^3/s^2}$ and $R_E = 6378$ km, compute the mean motion $n$ for a circular orbit at altitude $h = 650$ km, then substitute that $n$ into the synchronization condition (5.5) together with $J_2 = 1.083\times10^{-3}$ and $\dot\Omega_{\mathrm{sun}} = 1.991\times10^{-7}\ \mathrm{rad/s}$ to find $\cos i$ and the sun-synchronous inclination $i$. [expect $n \approx 1.07\times10^{-3}\ \mathrm{rad/s}$, $\cos i \approx -0.14$, $i \approx 98^\circ$]

3. The chapter states that the required sun-synchronous inclination rises from about $96.3^\circ$ at 200 km to about $98.6^\circ$ at 800 km. Using the consolidated form (5.7), in which $\cos i \propto a^{7/2}$, determine by what factor $\cos i$ changes between these two altitudes, taking $R_E = 6378$ km. Confirm that the sign of the change is consistent with the inclination moving farther from $90^\circ$ as altitude increases, and explain in one or two sentences why the $a^2$ term in (5.5) dominates the falling $n$.

4. A thermal requirement forces the reference orbit to be lowered from 650 km to 600 km. Using (5.7), estimate the resulting change in $\cos i$ and hence the change in inclination, holding $\dot\Omega_{\mathrm{sun}}$, $J_2$, $R_E$, and $\mu$ at their chapter values. Comment on the design statement that, once altitude is set by a power or thermal constraint, the sun-synchronous inclination is fixed to within a small fraction of a degree rather than being a free parameter.
...
1. Starting from the projected-geometry result (6.6), $d_\perp(\theta) = r\sqrt{1 - \cos^2\beta\,\cos^2\theta}$, show that the boundary condition $d_\perp = R_E$ on the anti-solar half of the orbit leads to (6.7), and hence to the eclipse half-angle of (6.3). Then show that demanding a real solution for $\psi$ reproduces the critical beta angle of (6.1), so that $\beta^{*} = \arcsin(R_E/(R_E + h))$ is exactly the limit $\cos\psi = 1$ of the eclipse condition.

2. A circular orbit is placed at altitude $h = 650$ km, for which $r = 7021$ km and $R_E = 6371$ km. Using (6.1), confirm the critical beta angle stated in the chapter, then use (6.3) to evaluate the eclipse half-angle $\psi$ for a fixed beta angle of $\beta = 60^\circ$ (a value below $\beta^{*}$, so the orbit does enter shadow). Convert this arc to a sunlit fraction with (6.2). [expect $\beta^{*} \approx 65^\circ$ and $f_{\mathrm{sun}}$ somewhat below 1]

3. For the same orbit at $h = 650$ km, use Kepler's third law (6.8) with $\mu_\oplus = 3.986 \times 10^{5}\ \mathrm{km^3\,s^{-2}}$ to estimate the orbital period $T$, and then use (6.9) together with your $\psi$ from Problem 2 to estimate the eclipse duration $t_E$ in minutes at $\beta = 60^\circ$. State, using the worked example of the chapter, what $t_E$ becomes once the dawn-dusk orbit holds $|\beta|$ above $\beta^{*}$ at all times.

4. The dawn-dusk orbit keeps $|\beta|$ oscillating between $90^\circ - \varepsilon$ and $90^\circ$ over the year, with $\varepsilon = 23.44^\circ$, so its lowest beta angle is $66.6^\circ$. Treating altitude as the free variable, determine by how much the altitude $h$ would have to be reduced before the critical beta angle $\beta^{*}$ of (6.1) rises to meet this $66.6^\circ$ lower bound, that is, the altitude at which the year-round eclipse-free margin just vanishes. By what factor does the quantity $R_E/(R_E + h)$ change between $h = 650$ km and this limiting altitude?
...
1. Starting from the dose-depth law (8.1) and its time integral (8.2), show that the shielding thickness at which the attenuated electron term equals a fraction $\epsilon$ of the proton-and-bremsstrahlung floor is the closing-thickness expression (8.3), $x^{*} = \lambda \ln\!\left( D_0 / (\epsilon\, D_\infty) \right)$. Then show that adding one further attenuation length $\lambda$ of aluminium beyond $x^{*}$ reduces the residual electron term by the factor $e^{-1}$, and explain in one or two sentences why this is the quantitative statement of the chapter's claim that mass past a centimetre or two buys almost no margin.

2. Using the floor estimate (8.4), $D_\infty \approx 212$ rad(Si) yr$^{-1}$, and the five-year cumulative dose of approximately 1.06 krad(Si) at $x = 10$ mm from (8.2), estimate the electron-attenuation prefactor $D_0$ implied by the chapter's calibration with $\lambda \approx 1.7$ mm. Treat the 1.06 krad(Si) as the full five-year dose and the floor as $212$ rad(Si) yr$^{-1}$ acting over the same five years, and solve (8.2) for $D_0$. Comment on the sign and magnitude of the residual electron term at 10 mm relative to the floor.

3. The reference mission's high-bandwidth memory first showed irregularities at 2 krad(Si) and exhibited no hard failures up to 15 krad(Si). Using the cumulative-dose law (8.2) with the constants determined in Problem 2 (or, equivalently, with the floor-dominated approximation $D(x,T) \approx D_\infty T$ valid for $x \ge 10$ mm), estimate the mission lifetime at $x = 10$ mm of aluminium that would be required to reach each of these two thresholds. By what factor do the two lifetimes differ, and how does that factor compare with the chapter's stated order-of-magnitude margin to the no-hard-failure level?

4. The throughput tax is given by (8.5), $\tau = c_{\mathrm{ECC}}\, \sigma_{\mathrm{SEU}}\, \phi\, N_{\mathrm{bits}} / r_{\mathrm{cyc}}$. Treating $c_{\mathrm{ECC}}$, $\sigma_{\mathrm{SEU}}$, $N_{\mathrm{bits}}$, and $r_{\mathrm{cyc}}$ as fixed by the hardware, show that $\tau$ scales linearly with the particle flux $\phi$. The chapter states that the tax peaks in the South Atlantic Anomaly and the polar horns and falls to a small floor elsewhere, consistent with a bounded ten to twenty percent figure. If the orbit-averaged tax is to be held at the lower end of that band when the flux inside the anomaly is some multiple of the quiescent flux, by what factor must the quiescent-region flux be smaller than the anomaly flux for the time-averaged tax to remain within the bounded band, expressed symbolically in terms of the fraction of orbital time spent in each region.
...
1. Starting from the binomial zero-impact probability in Equation (9.2), show that in the limit of infinitely many short slices at fixed product $mp = \Phi A N t$ the cluster catastrophic probability reduces to Equation (9.1). State precisely where the chapter's independence assumption enters, and use Equation (9.5) to argue that the same exponent $N\lambda_1$ is obtained whether one groups the fleet into a single Poisson process or treats each satellite as an independent survivor.

2. Using the per-satellite exposure $\lambda_1 = \Phi A t$ defined just above Equation (9.5) and the chapter's reference values $\Phi = 3\times10^{-5}\ \mathrm{m^{-2}yr^{-1}}$, $A = 15\ \mathrm{m^2}$, and $t = 5\ \mathrm{yr}$, estimate the catastrophic-impact probability for a single satellite over the mission. Then form the fleet mean for $N = 81$ from Equation (9.3) and confirm it matches the worked value in Equation (9.4). [expect $\Lambda$ of order $10^{-1}$]

3. The chapter quotes a flux model band corresponding to a probability range from six to forty-six percent about the nominal seventeen percent. Treating the nominal mean as $\Lambda = 0.182$ from Equation (9.4) and holding $A$, $N$, and $t$ fixed, determine the two flux values $\Phi$ that reproduce the lower and upper ends of that probability band using Equation (9.1). Report each as a multiple of the nominal flux.

4. A revised design halves the tumbling-averaged silhouette to $A = 7.5\ \mathrm{m^2}$ while leaving $\Phi$, $N$, and $t$ unchanged. Using the linearity of Equation (9.3), find the new mean $\Lambda$ and, using Equation (9.1), the new cluster probability $P(K\ge 1)$. State the factor by which the mean changes, and explain with reference to the design implication stated after Equation (9.5) why this is equivalent to halving either the node count or the mission duration.
...
1. The chapter states that the radiator area is linear in heat load, with no economy of scale. Starting from the steady-state panel energy balance (10.4), derive the sizing relation (10.3) explicitly, stating where the cosmic-microwave-background sink term is dropped and justifying that step with the chapter's estimate of $T_{\mathrm{CMB}}^4/T^4$. Then show that the area per unit power, $A_{\mathrm{rad}}/Q$, is independent of $Q$, and explain in one sentence why this rules out a per-watt cooling advantage for a larger plant.

2. Using (10.3) and the worked example reference values ($\varepsilon = 0.90$, $n_s = 2$, $f_p = 0.12$, operating temperature $T = 20\ \mathrm{C}$, with $\sigma$ defined in (10.2)), estimate the radiator area required to reject a heat load of $Q = 2.5\ \mathrm{MW}$. Report the per-side flux $\varepsilon\sigma T^4$ and the net flux $\varepsilon\sigma T^4 n_s (1 - f_p)$ that you use, and compare your result to the industry rule of roughly 1,200 m² per megawatt quoted in the chapter. [expect order $10^3\ \mathrm{m^2}$]

3. The chapter reports that running the silicon hotter buys back radiator area through the $T^4$ dependence, formalized by the sensitivity relation (10.5). Using (10.5), estimate the fractional change in required area if the operating temperature is raised from $20\ \mathrm{C}$ to $60\ \mathrm{C}$, working in absolute temperature. Then, to test the accuracy of the linearized estimate, recompute the area ratio directly from (10.3) at the two temperatures and state by what factor the area changes. Comment on whether the linear approximation in (10.5) over- or under-estimates the saving across this finite temperature step.

4. The passive thermal chain imposes a second limit independent of radiator size. Using the conductive chain model $T_j = T_{\mathrm{rad}} + Q\,R_{th}$ and the passive ceiling (10.6) with the chapter's junction budget $\Delta T_{\max} = 125\ \mathrm{C}$, determine the maximum passively coolable chip power for the baseline chain $R_{th} = 0.30\ \mathrm{K/W}$, and confirm it matches the threshold in (10.7). Then size the interface: by what factor must $R_{th}$ be reduced so that a single device dissipating $1200\ \mathrm{W}$ can be held within the same junction budget, and state the corresponding target value of $R_{th}$.
...
1. Starting from the multiplicative radiation-degradation model (11.2), show that the implied annual degradation rate is $d = 1 - (\eta_{\mathrm{life}}/\eta_{\mathrm{BOL}})^{1/N}$, and that for small $d$ the five-year end-of-life efficiency is approximately $\eta_{\mathrm{life}} \approx \eta_{\mathrm{BOL}}(1 - Nd)$. Using $\eta_{\mathrm{BOL}} = 0.296$, $\eta_{\mathrm{life}} = 0.275$, and $N = 5$, evaluate the exact rate from (11.2) and compare it with the linearized estimate, commenting on the size of the discrepancy.

2. Using the load relation (11.1) with the chapter's reference values ($S_0 = 1361\ \mathrm{W/m^2}$, $\kappa_{\mathrm{pack}} = 0.85$, $\eta_{\mathrm{path}} = 0.88$, $k_T = -0.0023\ \mathrm{per\ C}$, $\eta_{\mathrm{life}} = 0.275$), estimate the power delivered to the load per square metre of array at a beginning-of-life operating temperature of 28 C. Then repeat at the 70 C end-of-life point and report the ratio of the two delivered-power densities. [expect the 70 C value to be near the $253\ \mathrm{W/m^2}$ of the worked example]

3. Size the array for a $5\ \mathrm{MW}$ load using the sizing closure (11.6) at the end-of-life delivered density of $253\ \mathrm{W/m^2}$. Then, treating the area as inversely proportional to $\eta(T,\mathrm{life})$ through (11.6) and (11.3), determine by what factor the required area changes if the panel can be operated at 40 C instead of 70 C, with all other quantities held fixed. [expect order $10^4\ \mathrm{m^2}$ for the baseline area]

4. Using the radiative-balance result (11.5) with $\alpha = 0.90$, $\varepsilon_f + \varepsilon_b = 1.6$, $\sigma = 5.67\times 10^{-8}\ \mathrm{W\,m^{-2}\,K^{-4}}$, $S_0 = 1361\ \mathrm{W/m^2}$, and $\kappa_{\mathrm{pack}} = 0.85$, carry out one fixed-point iteration for the equilibrium temperature: begin from the 70 C operating point, evaluate $\eta$ from (11.1) at that temperature, substitute into (11.5) to obtain an updated $T$, and then re-evaluate $\eta$ at the updated temperature. Report the temperature after the iteration and state whether a hotter panel raises or lowers the equilibrium temperature, consistent with the coupling noted after (11.5).
...
1. Starting from the volume integral (12.2), show that the principal moment about the $y$ axis of a uniform rectangular solid is $I_y = \tfrac{m}{12}(d_x^2 + d_z^2)$. Identify the squared distance from the $y$ axis that replaces $y^2 + z^2$, carry out the three integrations using $\int_{-d/2}^{d/2} s^2\,ds = d^3/12$, and confirm that your result is the cyclic permutation of (12.3). Then add the three principal moments and state the form of $I_x + I_y + I_z$ in terms of $d_x, d_y, d_z$.

2. A deployed solar wing of mass $m = 30\ \mathrm{kg}$ and length $L = 6\ \mathrm{m}$ is mounted with its centroid at an offset $o = 4\ \mathrm{m}$ from the bus axis. Using the parallel-axis relation (12.4), evaluate the moment of inertia this single wing contributes about the bus axis. Then report separately the centroidal term $mL^2/12$ and the offset term $mo^2$, and state which dominates. Compare the total with the chapter's statement that the maximum principal moment of the reference satellite is on the order of a few hundred kilogram-meters squared. [expect the offset term to dominate]

3. Using the minimum-area relation (12.6), size a single primary member. Take a tributary mass $m = 80\ \mathrm{kg}$, the chapter's quasi-static load factor $g_{\mathrm{load}} = 8.5$, standard gravity $g_0 = 9.81\ \mathrm{m\,s^{-2}}$, an aluminium yield strength $\sigma_y = 270\ \mathrm{MPa}$, and a factor of safety $F_s = 1.25$. Compute the minimum cross-sectional area $A_{\mathrm{member,\,min}}$ in square millimeters. Then, using (12.5), state by what factor the strength margin $\mathrm{MS}$ above zero increases if the actual member area is doubled relative to $A_{\mathrm{member,\,min}}$.

4. The chapter's design implication states that inertia grows with the square of panel length while the launch-driven member area grows only linearly with mass. Treat a deployed panel as a uniform plate whose mass is proportional to its area, so that for fixed thickness and offset the mass scales as $m \propto L^2$. Using (12.4) with the offset term neglected, and (12.6), determine the factor by which the centroidal inertia contribution $mL^2/12$ changes and the factor by which the required member area $A_{\mathrm{member,\,min}}$ changes when the panel length $L$ is increased by 50 percent. Use the comparison to justify the chapter's recommendation that the mass budget be tightened at the geometry stage rather than at member sizing.
...
1. Starting from the general gravity-gradient torque (13.2), $\boldsymbol{\tau}_{\mathrm{gg}} = 3 n^2\, \hat{\mathbf{r}} \times (\mathbf{I}\,\hat{\mathbf{r}})$, resolve the local vertical for a single-axis pitch offset $\theta$ as $\hat{\mathbf{r}} = (\sin\theta,\, 0,\, \cos\theta)$ in the principal frame and show that the only surviving torque component reduces to (13.3), $\tau_{\mathrm{gg}} = \tfrac{3}{2}\, n^2\, (I_z - I_x)\, \sin 2\theta$. State the two pitch angles in $[0^\circ, 90^\circ]$ at which the torque vanishes and the angle at which it is maximum, and explain why this result motivates flying the bus with its minimum-inertia axis along the local vertical.

2. Using the mean motion at the reference altitude from (13.4), $n \approx 1.07\times10^{-3}\ \mathrm{rad\,s^{-1}}$, and the peak gravity-gradient torque from (13.3), estimate the maximum disturbance torque ($\theta = 45^\circ$) for an inertia asymmetry $|I_z - I_x| = 100\ \mathrm{kg\,m^2}$. Confirm that the result is consistent with the order $10^{-4}\ \mathrm{N\,m}$ quoted in the chapter. [expect order $10^{-4}\ \mathrm{N\,m}$]

3. The pointing-loss factor is $L_{\mathrm{point}} = \exp(-2\theta_{\mathrm{err}}^2/\theta_{\mathrm{div}}^2)$, and its small-jitter linearization (13.5) gives $\theta_{\mathrm{err}} \le \theta_{\mathrm{div}}\sqrt{\epsilon/2}$ for a fractional power-loss budget $\epsilon$. For the diffraction-limited divergence $\Theta \approx 2.44\,\lambda/D$, take the full divergence as $\theta_{\mathrm{div}} = \Theta$ and size the maximum allowable residual jitter $\theta_{\mathrm{err}}$ for a link operating at $\lambda = 1.55\ \mu\mathrm{m}$ through an aperture $D = 0.10\ \mathrm{m}$, holding the pointing loss to $\epsilon = 0.10$. Compare your result to the two-arcsecond (roughly ten-microradian) attitude knowledge a star tracker provides, and state whether the bus alone can meet the requirement.

4. Two laser links are to be assessed with the same transmit aperture $D = 0.10\ \mathrm{m}$ and wavelength $\lambda = 1.55\ \mu\mathrm{m}$: a 150-m neighbor hop and a long inter-satellite hop. Using the divergence relation $\Theta \approx 2.44\,\lambda/D$, compute the beam footprint (diameter) at 150 m and confirm the chapter's claim that the neighbor link is geometrically trivial because the spot is millimeters wide. Then, using the linearized requirement $\theta_{\mathrm{err}} \le \theta_{\mathrm{div}}\sqrt{\epsilon/2}$ from (13.5), state by what factor the residual-jitter requirement tightens if a redesign doubles the aperture to $D = 0.20\ \mathrm{m}$ at fixed $\epsilon$, and explain why tighter beams force reliance on a fine-steering mirror slaved to the received beacon.
...
1. Starting from the spreading argument that leads to Eq. (14.2), show that the product of transmit and receive telescope gains for two identical circular apertures of diameter $D$, combined with the free-space loss of Eq. (14.1), gives a net range dependence $G_{\mathrm{tx}}G_{\mathrm{rx}}/L_{\mathrm{fs}} \propto (D^2/(\lambda R))^2$. Use this to explain, in terms of the exponents on $D$, $\lambda$, and $R$, why doubling each aperture diameter buys the same received-power improvement as halving the range.

2. Using the free-space-loss expression of Eq. (14.3), evaluate $L_{\mathrm{fs}}$ in decibels at the chapter's reference wavelength of 1550 nm for the two ranges named in the text, 1,000 km and 10,000 km. Confirm from your two values that the difference matches the twenty-decibel-per-decade slope stated in Eq. (14.3), and comment on how this slope is consistent with the quoted jump from 34 mW to 3.41 W in required transmit power across the same decade of range.

3. The pointing loss of Eq. (14.4) is evaluated in the chapter at $\theta_e = 1\ \mu\mathrm{rad}$ against a divergence of $\theta_{\mathrm{div}} = 15\ \mu\mathrm{rad}$, giving about 0.04 dB. By what factor does the pointing loss in decibels change if pointing jitter degrades so that $\theta_e$ rises to $5\ \mu\mathrm{rad}$ while $\theta_{\mathrm{div}}$ is held fixed? Using the quadratic form of Eq. (14.4), state the general rule for how the decibel pointing loss scales with $\theta_e$, and use it to explain the chapter's statement that jitter approaching the divergence angle degrades the budget sharply.

4. Size the ground segment for cloud-cover availability. Using the site-diversity relation $1 - (1 - p_{\mathrm{clear}})^n$ from the chapter, and the chapter's statement that four stations reach roughly ninety-six percent, first infer the single-site clear-sky probability $p_{\mathrm{clear}}$ consistent with that figure. Then determine the smallest number of independent stations $n$ needed to raise the at-least-one-clear probability above ninety-nine percent. Separately, using the latency-crossover result of the chapter, state the fixed fractional time advantage of the vacuum mesh over fiber that follows from $n_{\mathrm{fiber}} = 1.4675$, and confirm it is the forty-seven percent quoted in the latency-crossover discussion.
...
1. Starting from the vis-viva equation (15.2), show that the speed at the apogee of the de-orbit transfer ellipse can be written as $v_{\mathrm{apo}} = v_{c1}\sqrt{2 r_2/(r_1 + r_2)}$, where $v_{c1} = \sqrt{\mu/r_1}$ is the circular speed at the initial radius and $a_t = (r_1 + r_2)/2$ is the transfer semi-major axis. Then take the limit $r_2 \to r_1$ and interpret the resulting value of $\Delta v_{\mathrm{deorbit}}$ from (15.1) physically.
...
1. Starting from the constant-hazard reliability of Eq. (16.1), show that the median time to failure of a unit is $t_{1/2} = \mathrm{MTBF}\,\ln 2$, and confirm using (16.4) that this median is strictly smaller than the mean lifetime. State in one sentence why a constant hazard produces a median that lies below the mean.

2. Using Eq. (16.1) and the bus figures in the chapter, estimate the five-year survival probability of a unit whose mean time between failures is 100,000 hours. Then, using the mean-capacity bound (16.7), estimate the deployed count required to hold an availability of $A = 0.95$ for $n_{\mathrm{required}} = 75$ nodes at this per-unit reliability. [expect a deployed count on the order of 100 units]

3. Using the renewal balance of Eq. (16.6), size the steady-state annual replacement count for a constellation of 120 deployed units at a design life of $L = 7$ years, and express the result as a percentage of the fleet per year. By what factor does this annual replacement count change if the design life is extended from five years to ten years at fixed fleet size?

4. The chapter notes that the expected number of survivors is $\mathbb{E}[X] = np$ with variance $np(1-p)$ from Eq. (16.5). For $n = 81$ deployed units at $p = 0.90$, compute $\mathbb{E}[X]$ and the standard deviation, and determine how many standard deviations the required count $k = 70$ sits below the mean. Using this margin, comment on why raising the per-unit reliability from $p = 0.90$ to $p = 0.95$ produces the large gain in the tail sum (16.2) reported in the chapter.
...
1. The chapter writes the model-FLOPs utilization and radiation availability as $\mathrm{MFU} = 1 - \ell_u$ and $a_r = 1 - \ell_r$ and states in (17.2) that the throughput penalties add to first order. Starting from the exact delivered rate (17.1), expand the product $(1 - \ell_u)(1 - \ell_r)$ and show that the leading correction dropped in passing to (17.2) is the single term $\ell_u \ell_r$. Then derive a general expression for the fractional error of the additive approximation (17.2) relative to the exact (17.1), and state in one sentence why the approximation is most trustworthy when one loss fraction dominates the other.

2. Using the worked-example reference values of the chapter, $\mathrm{MFU} = 0.10$ for inference decode and $a_r = 0.95$, apply (17.1) to report the delivered fraction of installed peak. Then apply (17.7) to state the factor by which the unit cost of useful work is inflated relative to a peak-rated quote, and apply (17.6) to convert a compute-direct overhead ratio of $1.2$ into an effective power-usage effectiveness. [expect a delivered fraction near 0.095 and a cost inflation near a factor of 10]

3. Consider the throttled fixed point (17.5). Two accelerators share the same throttle gain $\beta$, the same junction-to-radiator resistance $R_{\theta}$, the same requested utilization $u_0$, and operate over the same radiator temperature $T_{\mathrm{rad}}$, but one is the 700-W part and the other is a low-power part as contrasted in the chapter. Using (17.5), show that the throttled effective utilization depends on the part only through the product $\beta R_{\theta} P_{\mathrm{peak}}$ in the denominator. By what factor does the denominator of (17.5) change if $P_{\mathrm{peak}}$ is reduced by a factor of seven while $\beta$, $R_{\theta}$, and $u_0$ are held fixed, and what does this imply for the effective utilization of the lower-power part relative to the high-power part?

4. The chapter states that chip-level active cooling acts on (17.5) by lowering the junction-to-radiator thermal resistance $R_{\theta}$. For a fixed high-power accelerator with given $u_0$, $\beta$, $T_{\mathrm{rad}}$, and $P_{\mathrm{peak}}$ in the throttling regime, derive from (17.5) the value of $R_{\theta}$ required to recover a target throttled utilization $u_{\mathrm{eff}}^{\star}$, expressed in terms of the chapter's symbols. Then, using your result, explain why design effort spent lowering $R_{\theta}$ returns more delivered work for a high-$P_{\mathrm{peak}}$ part than for a low-$P_{\mathrm{peak}}$ part, and connect this to the design implication that utilization is the largest single factor for an inference-dominated orbital data center.
...
1. The mass fixed point of (18.3) collects every kilogram fixed by physics into $m_0$ and lets the structure scale as $f_{\mathrm{str}}\,m_{\mathrm{dry}}$. Starting from (18.3), show that the closed form (18.4) follows, and then show that the marginal sensitivity of the dry mass to the payload-and-bus term is $\partial m_{\mathrm{dry}} / \partial m_0 = 1/(1 - f_{\mathrm{str}})$, so that the structural amplifier acts identically on the closed mass and on its derivative. Comment on the behavior of both quantities as $f_{\mathrm{str}} \to 1$ and relate this to the design implication stated in the chapter.

2. Using the worked example values of the chapter, namely a payload-and-bus term $m_0 \approx 224$ kg and a structural fraction $f_{\mathrm{str}} = 0.20$, evaluate the structural amplification $1/(1 - f_{\mathrm{str}})$ of (18.4) and the resulting dry mass $m_{\mathrm{dry}}$. Confirm that the result lies inside the reference four-chip range stated in the chapter. [expect $m_{\mathrm{dry}}$ of order $280$ kg]

3. The chapter notes that the structural and contingency fractions multiply the entire budget, so savings there propagate through the whole design. Using (18.4), determine by what factor the dry mass changes if the structural fraction is reduced from $f_{\mathrm{str}} = 0.20$ to $f_{\mathrm{str}} = 0.12$, holding the payload-and-bus term $m_0$ fixed. Express the answer as the ratio of the two amplification factors, and state in words why trimming a fraction is more effective than trimming any single subsystem mass inside $m_0$.

4. Once the dry mass is fixed, the wet mass follows from the propulsive amplifier of (18.6), $m_{\mathrm{wet}} = m_{\mathrm{dry}}\exp\!\left(\Delta v / (I_{sp}\,g_0)\right)$. Treating $m_{\mathrm{dry}}$ as the closed value from (18.4), show that the wet mass can be written as a product of the two amplifiers acting on the payload-and-bus term,
$$ m_{\mathrm{wet}} = \frac{m_0}{1 - f_{\mathrm{str}}}\,\exp\!\left(\frac{\Delta v}{I_{sp}\,g_0}\right), $$
and identify which factor is the structural amplification and which is the propulsive amplification. Then explain, using (18.5), why a higher dry mass at fixed drag area $A_d$ and drag coefficient $C_D$ raises the ballistic coefficient $\beta$ and therefore slows the natural decay.
...
1. Starting from the first-order Taylor expansion (19.2) and the general correlated variance result (19.3), show that for a pure power-law monomial $y = c\,\prod_{j} x_j^{a_j}$ with independent inputs the linear-propagation formula (19.1) reduces to the relative-error rule (19.4). In your derivation, evaluate the partial derivative $\partial y/\partial x_j$ explicitly and state where the independence assumption enters.
...
4. Using the useful-compute chain (21.6) and the proportionality $\mathrm{LCOE} \propto 1/(\eta_{\mathrm{th}}\, u\, A_{\mathrm{op}})$ implied by (21.1) and (21.6), determine by what factor the levelized cost of compute changes if a chip throttles from $\eta_{\mathrm{th}} = 1.0$ to $\eta_{\mathrm{th}} = 0.5$ while $u$ and $A_{\mathrm{op}}$ are held fixed. Then size the throttling factor $\eta_{\mathrm{th}}$ required to offset a drop in model-FLOPs utilization from $u = 0.6$ to $u = 0.4$ so that the annual useful PFLOP-hours of (21.6) are unchanged, holding $P_{\mathrm{peak}}$ and $A_{\mathrm{op}}$ fixed. [expect the cost to double in the first part]
...
1. Starting from the Wright relation (22.1) and the definition of the learning rate as the fractional price reduction per doubling of cumulative launched mass, show that the exponent satisfies $b = -\ln(1-L)/\ln 2$ as in (22.2). Then show that the cumulative-mass ratio required to reach a target price, equation (22.3), can be written as $m/m_0 = (C_0/C^\ast)^{1/b}$, and explain why a learning rate that is smaller than assumed makes this ratio larger for a fixed target price.

2. Using (22.2), evaluate the learning exponent $b$ and the inverse exponent $1/b$ at the two ends of the sampled learning-rate range, $L = 0.15$ and $L = 0.25$. For each case, use (22.3) to find the factor by which cumulative launched mass must rise to drive the effective price down by a factor of ten toward the 200 dollars per kilogram target. Comment on how much the required mass factor changes across the sampled learning-rate range. [At $L=0.20$ the chapter gives a factor near $1.3\times 10^3$.]

3. The chapter's worked estimate takes the effective price to fall by roughly a factor of ten to approach the target. Treat the 2026 effective price as the reference price $C_0$ in (22.1) and use the sampled range of 1,200 to 3,500 dollars per kilogram. For each end of that range, compute the actual price ratio $C_0/C^\ast$ needed to reach $C^\ast = 200$ dollars per kilogram, then use (22.3) with $L = 0.20$ to find the required cumulative-mass ratio $m/m_0$ at each end. Discuss how the choice of starting price alone, holding the learning rate fixed, shifts the required cumulative mass.

4. Using the required cumulative-mass ratio from the chapter's worked estimate ($m/m_0 \approx 1.3\times 10^3$ at $L = 0.20$) together with the cadence model (22.4), size the number of flights needed to accumulate the required mass. Take a payload of $p = 200$ tonnes per launch, take the reference mass $m_0$ to be the additional cumulative mass already implied near the start of the ramp, and use the chapter's stated figure of roughly 370,000 tonnes of additional cumulative launched mass to the target. Estimate the number of launches this implies at 200 tonnes per flight, and compare your figure with the chapter's order of 1,800 Starship launches. Then, holding the required mass fixed, state by what factor the number of launches changes if the payload is reduced from 250 tonnes to 150 tonnes, the two ends of the sampled payload range.
...
1. Starting from the power-summation rule of the chapter, show that the electrical power-usage effectiveness can be written purely in terms of the contingency margin, the compute load, and the subsystem sum, namely that combining (26.1) and (26.2) gives $\mathrm{PUE}_{\text{elec}} = (1+m)\left(1 + \sum_i P_i / P_{\text{IT}}\right)$. Using this form, state by inspection how $\mathrm{PUE}_{\text{elec}}$ responds in the limit where the fixed subsystem allocations become negligible compared with the accelerator draw, and confirm that the reference design point reproduces the tabulated value of 1.58.
...
1. Starting from the rotating-frame acceleration (28.2) with $\mathbf{n} = n\,\hat{\mathbf{z}}$, show that expanding the radial gravitational term $-\mu/(r_0+x)^2$ to first order in $x$ gives $-n^2 r_0 + 2 n^2 x$, and that the constant part cancels against the centrifugal term acting on the reference radius. Identify which term in (28.1) the surviving $2 n^2 x$ contribution corresponds to, and confirm that the cross-track equation $\ddot z + n^2 z = a_z$ describes simple harmonic motion at the orbital rate $n$.

2. Using the secular along-track growth (28.3), estimate the time at which the along-track offset reaches the 150-m spacing budget for a differential drag acceleration $\delta a_{\mathrm{drag}} = 1\times 10^{-7}\ \mathrm{m\,s^{-2}}$, following the inversion in (28.4). Then repeat the estimate for the case in which differential drag is ten times larger, $\delta a_{\mathrm{drag}} = 1\times 10^{-6}\ \mathrm{m\,s^{-2}}$, and report the new budget time. [expect $t_{\mathrm{budget}}$ of order $10^4$ s in the first case]

3. The chapter states that differential atmospheric drag varies by a factor of one hundred over the solar cycle. Using the quadratic scaling of (28.3) and the inversion of (28.4), determine by what factor the budget time $t_{\mathrm{budget}}$ changes when $\delta a_{\mathrm{drag}}$ increases by a factor of one hundred from solar minimum to solar maximum. State whether the formation must close its relative-navigation and thrust loop more or less frequently at solar maximum, and explain in one sentence why this sets the maneuver cadence rather than the total propellant.

4. Size the maximum tolerable constant differential along-track acceleration $\delta a$ such that the along-track offset (28.3) remains below the 150-m spacing budget over a control interval of one orbital period $T = 2\pi/n$. Express your answer in terms of $\Delta y_{\max}$ and $n$, and then evaluate it for a reference orbit with mean motion $n = 1.1\times 10^{-3}\ \mathrm{rad\,s^{-1}}$.
