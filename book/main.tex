...
\bottomrule\noalign{}
\endlastfoot
ADCS & attitude determination and control & LCOE & levelized cost of
compute \\
AM0 & air-mass-zero solar spectrum & LEO & low Earth orbit \\
BOL / EOL & beginning / end of life & MFU & model-FLOPs utilization \\
CAPEX / OPEX & capital / operating expenditure & MTBF & mean time
between failures \\
CRF & capital-recovery factor & NPV / IRR & net present value / internal
rate of return \\
ECC & error-correcting code & PUE & power-usage effectiveness \\
FSO / ISL & free-space optical / inter-satellite link & SAA & South
Atlantic Anomaly \\
FSPL & free-space path loss & SEU & single-event upset \\
GPU / TPU & graphics / tensor processing unit & SSO & sun-synchronous
orbit \\
HBM & high-bandwidth memory & TID & total ionizing dose \\
IR & infrared & TIM & thermal interface material \\
\(J_2\) & second zonal gravity harmonic & TMR & triple modular
redundancy \\
RAAN & right ascension of the ascending node & TRL &
...
This book uses SI units throughout unless a quantity is stated
otherwise; temperatures are given in degrees Celsius or kelvin as
marked, and angles in degrees unless radians are specified. The Earth
gravitational parameter is \(\mu = G M_\oplus\), and the standard
physical constants are those listed above. Display equations are
numbered by chapter, so a tag such as (10.3) refers to the third
numbered equation of Chapter 10. Vectors are set in bold and an overdot
denotes a time derivative. Numbers that are demonstrated or computed
from the model are stated directly; forward-looking quantities, such as
future launch prices and parity dates, are flagged as projections and
given with uncertainty bands rather than as single values.
Cross-references name the chapter by its number.
...
the heat that a radiator\index{radiator} can reject to deep space, and the balance
...
Stefan-Boltzmann\index{Stefan-Boltzmann} law,
...
cluster of approximately one-kilometer radius, in a dawn-dusk\index{dawn-dusk}
sun-synchronous\index{sun-synchronous} low-Earth orbit at a mean altitude of 650 km, with the
...
launch, and validated formation station-keeping\index{station-keeping}. None of the latter has
been demonstrated.
...
radiation. For a gray emitter of area \(A\) and emissivity\index{emissivity}
...
margin of \(13.7\ ^\circ\mathrm{C}\). The case closes, but not by much.
...
\item
  \textbf{Margin as a function of chain resistance.} Starting from the
  junction relation (2A.4) and the radiator balance (2A.2), show that
  the junction margin against the limit \(T_{j,\mathrm{lim}}\) can be
  written as
  \[\Delta T = T_{j,\mathrm{lim}} - \left(\frac{Q_{\mathrm{total}}}{\varepsilon\,\sigma\,A\,n_s}\right)^{1/4} - P_{\mathrm{chip}}\,R_{\mathrm{th}}.\]
  Using this expression, determine the chain resistance
  \(R_{\mathrm{th}}\) at which the margin vanishes, given the chapter's
  reference values \(Q_{\mathrm{total}} = 1450\ \mathrm{W}\),
  \(\varepsilon = 0.85\), \(A\,n_s = 4\ \mathrm{m^2}\),
  \(P_{\mathrm{chip}} = 300\ \mathrm{W}\), and
  \(T_{j,\mathrm{lim}} = 125\ ^\circ\mathrm{C}\). State how much
  resistance headroom the baseline chain of (2A.3) retains.
\item
  \textbf{Sizing the radiator for a colder panel.} The closing argument
  of the chapter notes that radiator area is set by the fourth power of
  the chosen panel temperature. Using the sizing inversion (2A.9) with
  \(Q_{\mathrm{total}} = 1450\ \mathrm{W}\) and \(\varepsilon = 0.85\),
  compute the single-sided emitting area \(A\,n_s\) required to settle
  the panel at \(T_{\mathrm{rad}} = 0\ ^\circ\mathrm{C}\) rather than at
  the baseline \(21.3\ ^\circ\mathrm{C}\). By what factor does the
  required area change relative to the baseline four square meters, and
  using (2A.4) by how many degrees does the junction margin improve if
  the chain resistance is held fixed at \(0.300\ \mathrm{K/W}\).
\item
  \textbf{Quartic response of the radiator to load.} Using the
  cold-background balance (2A.2), show that for fixed \(\varepsilon\),
  \(A\), and \(n_s\) the radiator temperature obeys
  \(T_{\mathrm{rad}} \propto Q_{\mathrm{total}}^{1/4}\), and confirm the
  chapter's statement that doubling the load raises \(T_{\mathrm{rad}}\)
  by the factor \(2^{1/4}\). Then evaluate the absolute temperature rise
  of the baseline panel (starting from
  \(T_{\mathrm{rad}} = 21.3\ ^\circ\mathrm{C} = 294.45\ \mathrm{K}\)) if
  \(Q_{\mathrm{total}}\) doubles to \(2900\ \mathrm{W}\), and state the
  resulting reduction in junction margin using (2A.4) with
  \(P_{\mathrm{chip}} = 300\ \mathrm{W}\) and
  \(R_{\mathrm{th}} = 0.300\ \mathrm{K/W}\).
\item
  \textbf{Warm-sink sensitivity bound.} Equation (2A.8) gives the
  sensitivity of the radiator temperature to the sink temperature as
  \(\partial T_{\mathrm{rad}}/\partial T_{\mathrm{sink}} = (T_{\mathrm{sink}}/T_{\mathrm{rad}})^3\).
  Taking the baseline panel temperature
  \(T_{\mathrm{rad}} = 294.45\ \mathrm{K}\) as approximately fixed, use
  this sensitivity to estimate the first-order rise in
  \(T_{\mathrm{rad}}\) produced by a sink at
  \(T_{\mathrm{sink}} = 250\ \mathrm{K}\) acting over that interval, and
  discuss in light of the cold-background assumption behind (2A.2)
  whether the baseline margin of \(13.7\ ^\circ\mathrm{C}\) from (2A.4)
  would survive such a sink. {[}expect a sensitivity of order \(0.6\){]}
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Top view of the reference cluster: about eighty satellites within a one-kilometre circle, linked by laser at roughly one hundred and fifty metre spacing.}]{schematics/diagram_constellation.png}}
...
formal failure-modes analysis, and runs of the official agency debris\index{debris}
...
roughly \(150\ \mathrm{m}\). The uniform area-per-satellite estimate is
an upper bound on the typical separation; the quoted \(150\ \mathrm{m}\)
reflects a denser central arrangement than a uniform fill, since the
satellites are clustered rather than spread evenly over the disc. The
...
where \(\mu\) is the Earth gravitational parameter\index{gravitational parameter}. The dawn-dusk
...
coefficient, and \(i\) the inclination\index{inclination}. Equation (3.4) shows that for a
...
and an explicit technology-readiness\index{technology-readiness} assessment that locates each
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Geometry of a Keplerian orbit: Earth at one focus, the major axis 2a from apogee to perigee, and the satellite at radius r and true anomaly theta.}]{schematics/sch_orbit_geometry.png}}
\caption{Geometry of a Keplerian orbit: Earth at one focus, the major
axis 2a from apogee to perigee, and the satellite at radius r and true
anomaly theta.}
\end{figure}
...
conservation yields the vis-viva\index{vis-viva} relation
...
cases of the two-body\index{two-body} problem. Newton's law of gravitation and the
...
shows that the specific angular momentum\index{angular momentum} is conserved,
...
eccentricity\index{eccentricity} (Laplace-Runge-Lenz) vector, yields the trajectory as a
...
reference orbit \(\Delta\lambda \approx 24.5^\circ\) per revolution, and
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the circular-speed relation (4.1) and the period
  relation (4.2), show that the orbital speed and period of a circular
  orbit are related by \(v = 2\pi a / T\), and use this to derive
  Kepler's third law in the form \(T^2 = (4\pi^2/\mu)\,a^3\). Then show
  that the specific energy (4.3) of a circular orbit can be written
  purely in terms of the speed as \(\mathcal{E} = -v^2/2\), and explain
  why an energy loss therefore forces a decrease in \(a\), consistent
  with the orbital-decay argument noted after (4.3).
\item
  Using the worked-example reference values
  (\(\mu = 3.986\times10^{14}\ \mathrm{m^3/s^2}\),
  \(a = 7.021\times10^6\) m) and the circular-speed and period relations
  (4.1) and (4.2), estimate the circular speed, the orbital period, and
  the number of orbits completed per day. Then, using
  \(\Delta\lambda = \omega_\oplus T\) with
  \(\omega_\oplus = 7.292\times10^{-5}\) rad/s, estimate the westward
  step of successive equator crossings. {[}expect \(v \approx 7.5\)
  km/s, \(T \approx 98\) min, near 15 orbits/day,
  \(\Delta\lambda \approx 24.5^\circ\) per revolution{]}
\item
  The reference orbit lies at altitude \(h = 650\) km. Consider raising
  the altitude to \(h = 1200\) km, with \(R_E\) taken as in the worked
  example so that \(a = R_E + h\). Using the period relation (4.2),
  determine by what factor the orbital period changes between the two
  altitudes, and using (4.1) determine by what factor the circular speed
  changes. State whether the higher orbit completes more or fewer orbits
  per day.
\item
  A circular-orbit constellation is to be sized so that its ground track\index{ground track}
  repeats after an integer number of nodal days. Using the repeat
  condition \(k\,T = d\,(2\pi/\omega_\oplus)\) from the ground-track
  discussion together with the period relation (4.2), derive an
  expression for the semi-major axis\index{semi-major axis} \(a\) that produces exactly \(k\)
  orbits in \(d\) nodal days, in terms of \(\mu\), \(\omega_\oplus\),
  \(k\), and \(d\). Then, using the reference values, evaluate \(a\) for
  the case \(k = 15\), \(d = 1\) and compare the result to the reference
  orbit at \(a = 7.021\times10^6\) m.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The dawn-dusk sun-synchronous orbit, with the orbital plane held on the day-night terminator so the satellite stays sunlit.}]{schematics/diagram_orbit.png}}
...
orbit. At 650 km, \(n = 1.072\times10^{-3}\ \mathrm{rad/s}\) and (5.5)
...
construction the corresponding nodal precession\index{nodal precession} is \(0.9856\) degrees
...
inclination condition (5.5) both require the mean motion\index{mean motion} \(n\), which
...
\(n = 1.072\times10^{-3}\ \mathrm{rad/s}\), the value already quoted in
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the secular disturbing function (5.3), show that
  Lagrange's planetary equation for the node (5.4) reduces to the
  precession rate (5.1) for a near-circular orbit. Carry out the
  derivative \(\partial\bar R/\partial i\) explicitly, demonstrate that
  the resulting \(\sin i\) in the numerator cancels the \(\sin i\) in
  the denominator of (5.4), and confirm that the surviving dependence is
  \(\cos i\) with the leading coefficient
  \(-\tfrac{3}{2}\,n\,J_2\,(R_E/a)^2\). State clearly where the
  approximation \(p = a(1-e^2) \approx a\) enters.
\item
  Using (5.6) with \(\mu = 3.986\times10^{14}\ \mathrm{m^3/s^2}\) and
  \(R_E = 6378\) km, compute the mean motion \(n\) for a circular orbit
  at altitude \(h = 650\) km, then substitute that \(n\) into the
  synchronization condition (5.5) together with
  \(J_2 = 1.083\times10^{-3}\) and
  \(\dot\Omega_{\mathrm{sun}} = 1.991\times10^{-7}\ \mathrm{rad/s}\) to
  find \(\cos i\) and the sun-synchronous inclination \(i\). {[}expect
  \(n \approx 1.07\times10^{-3}\ \mathrm{rad/s}\),
  \(\cos i \approx -0.14\), \(i \approx 98^\circ\){]}
\item
  The chapter states that the required sun-synchronous inclination rises
  from about \(96.3^\circ\) at 200 km to about \(98.6^\circ\) at 800 km.
  Using the consolidated form (5.7), in which
  \(\cos i \propto a^{7/2}\), determine by what factor \(\cos i\)
  changes between these two altitudes, taking \(R_E = 6378\) km. Confirm
  that the sign of the change is consistent with the inclination moving
  farther from \(90^\circ\) as altitude increases, and explain in one or
  two sentences why the \(a^2\) term in (5.5) dominates the falling
  \(n\).
\item
  A thermal requirement forces the reference orbit to be lowered from
  650 km to 600 km. Using (5.7), estimate the resulting change in
  \(\cos i\) and hence the change in inclination, holding
  \(\dot\Omega_{\mathrm{sun}}\), \(J_2\), \(R_E\), and \(\mu\) at their
  chapter values. Comment on the design statement that, once altitude is
  set by a power or thermal constraint, the sun-synchronous inclination
  is fixed to within a small fraction of a degree rather than being a
  free parameter.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The beta angle between the orbit plane and the Sun line, set against Earth's shadow; a high beta keeps the orbit out of eclipse.}]{schematics/diagram_beta.png}}
...
eclipse\index{eclipse} arc shrinks to zero.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 6.1 Critical beta angle versus altitude, and eclipse fraction versus beta at 650 km. A dawn-dusk orbit holds the beta angle high, so the satellite stays sunlit.}]{figures2/figA1_eclipse.png}}
\caption{Fig. 6.1 Critical beta angle versus altitude, and eclipse
fraction versus beta at 650 km. A dawn-dusk orbit holds the beta angle\index{beta angle}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the projected-geometry result (6.6),
  \(d_\perp(\theta) = r\sqrt{1 - \cos^2\beta\,\cos^2\theta}\), show that
  the boundary condition \(d_\perp = R_E\) on the anti-solar half of the
  orbit leads to (6.7), and hence to the eclipse half-angle of (6.3).
  Then show that demanding a real solution for \(\psi\) reproduces the
  critical beta angle of (6.1), so that
  \(\beta^{*} = \arcsin(R_E/(R_E + h))\) is exactly the limit
  \(\cos\psi = 1\) of the eclipse condition.
\item
  A circular orbit is placed at altitude \(h = 650\) km, for which
  \(r = 7021\) km and \(R_E = 6371\) km. Using (6.1), confirm the
  critical beta angle stated in the chapter, then use (6.3) to evaluate
  the eclipse half-angle \(\psi\) for a fixed beta angle of
  \(\beta = 60^\circ\) (a value below \(\beta^{*}\), so the orbit does
  enter shadow). Convert this arc to a sunlit fraction with (6.2).
  {[}expect \(\beta^{*} \approx 65^\circ\) and \(f_{\mathrm{sun}}\)
  somewhat below 1{]}
\item
  For the same orbit at \(h = 650\) km, use Kepler's third law (6.8)
  with \(\mu_\oplus = 3.986 \times 10^{5}\ \mathrm{km^3\,s^{-2}}\) to
  estimate the orbital period \(T\), and then use (6.9) together with
  your \(\psi\) from Problem 2 to estimate the eclipse duration \(t_E\)
  in minutes at \(\beta = 60^\circ\). State, using the worked example of
  the chapter, what \(t_E\) becomes once the dawn-dusk orbit holds
  \(|\beta|\) above \(\beta^{*}\) at all times.
\item
  The dawn-dusk orbit keeps \(|\beta|\) oscillating between
  \(90^\circ - \varepsilon\) and \(90^\circ\) over the year, with
  \(\varepsilon = 23.44^\circ\), so its lowest beta angle is
  \(66.6^\circ\). Treating altitude as the free variable, determine by
  how much the altitude \(h\) would have to be reduced before the
  critical beta angle \(\beta^{*}\) of (6.1) rises to meet this
  \(66.6^\circ\) lower bound, that is, the altitude at which the
  year-round eclipse-free margin just vanishes. By what factor does the
  quantity \(R_E/(R_E + h)\) change between \(h = 650\) km and this
  limiting altitude?
\end{enumerate}
...
energy balance under atmospheric drag\index{atmospheric drag} and integrates it to a mission
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Atmospheric drag slowly removes orbital energy, so the orbit spirals inward; the natural lifetime is about twelve years at moderate solar activity.}]{schematics/sch_decay_spiral.png}}
\caption{Atmospheric drag slowly removes orbital energy, so the orbit
spirals inward; the natural lifetime is about twelve years at moderate
solar activity.}
\end{figure}
...
reference bus, with an area-to-mass ratio of \(0.015\ \mathrm{m^2/kg}\)
and \(C_D = 2.2\), the natural lifetime at 650 km is approximately
\textbf{twelve years} at moderate solar activity, about two and a half
years at solar maximum, and roughly one hundred seventy years at solar
minimum (Fig. 7.1). The regulatory consequence is severe: the United
States Federal Communications Commission now requires disposal within
five years (Chapter 15), so only a satellite that happens to launch into
a solar maximum approaches passive compliance. \textbf{Active de-orbit
propulsion is therefore mandatory, not optional}, a conclusion that
propagates into the mass and economic budgets.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 7.1, Natural orbital lifetime versus altitude with the solar-cycle uncertainty band; at 650 km the realistic lifetime fails the five-year disposal rule.}]{figures/fig1_orbital_decay.png}}
...
\(B = C_D (A/m) = 2.2 \times 0.015 \approx 0.033\ \mathrm{m^2/kg}\) at
\(a_0 \approx 7028\ \mathrm{km}\) and
\(\mu = 3.986\times 10^{14}\ \mathrm{m^3/s^2}\), so that
\(\sqrt{\mu a_0} \approx 5.3\times 10^{10}\ \mathrm{m^2/s}\). With a
representative scale height of order tens of kilometres, (7.7) returns
lifetimes in the few-years-to-century range as the density \(\rho_0\) is
...
rule across the solar cycle, and the de-orbit\index{de-orbit} propellant\index{propellant} budget must be
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the orbit-averaged decay relation (7.3), show that the
  per-revolution contraction of the semi-major axis can be written
  \(\Delta a_{\mathrm{rev}} = -2\pi C_D (A/m)\,\rho\, a^2\). Use the
  circular-orbit period \(T = 2\pi\sqrt{a^3/\mu}\) and the circular
  speed \(v = \sqrt{\mu/a}\), and state the assumption about how
  \(\rho\) and \(a\) change over a single revolution that makes this
  step exact.
\item
  Using the closed-form lifetime estimate (7.7) with the reference-bus
  ballistic factor \(B = C_D(A/m) = 0.033\ \mathrm{m^2/kg}\),
  \(\mu = 3.986\times 10^{14}\ \mathrm{m^3/s^2}\),
  \(a_0 \approx 7028\ \mathrm{km}\), and a scale height
  \(H = 60\ \mathrm{km}\), estimate the natural lifetime for an ambient
  density \(\rho_0 = 1.0\times 10^{-13}\ \mathrm{kg/m^3}\). Then report
  the lifetime obtained when \(\rho_0\) is increased by a factor of one
  hundred to represent the swing from solar minimum to solar maximum
  described in the chapter, and state the factor by which the lifetime
  changes.
\item
  The chapter states that lifetime falls linearly with both the
  area-to-mass ratio and the ambient density (7.7). A design revision
  adds deployable radiators that raise the area-to-mass ratio from
  \(0.015\ \mathrm{m^2/kg}\) to \(0.0225\ \mathrm{m^2/kg}\) while the
  bus is simultaneously flown during a period when the ambient density
  is a factor of three higher than the reference value. By what overall
  factor does the natural lifetime change relative to the reference case
  at the reference density, with \(C_D\) held at \(2.2\)?
\item
  Using the scale-height definition (7.4) and the local exponential
  profile (7.5), derive an expression for the altitude interval
  \(\Delta h\) over which the atmospheric density falls by a factor of
  ten, in terms of the scale height \(H\). Then, taking
  \(H = 60\ \mathrm{km}\), estimate \(\Delta h\), and explain in one or
  two sentences why this exponential structure justifies the log-linear
  interpolation between the NRLMSIS density anchors used in the chapter.
\end{enumerate}
...
bounded throughput tax\index{throughput tax}.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Radiation shielding and single-event upsets: aluminium attenuates the accumulated dose, while error correction and checkpointing handle upsets.}]{schematics/diagram_shield.png}}
...
of total ionizing dose\index{total ionizing dose}, which is a shielding problem, and the stochastic
flipping of bits by energetic particles, which is a reliability\index{reliability} and
throughput problem.
...
2.7 times the 750 rad(Si) five-year requirement, and exhibited no hard
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 8.1 Cumulative five-year ionizing dose versus aluminium shielding, with the memory tolerance and the mission requirement marked. Ten millimeters of aluminium keeps the dose well below the first-irregularity threshold.}]{figures2/figA5_dose.png}}
...
therefore error-correcting\index{error-correcting} memory with checkpointing\index{checkpointing}, not full hardware
...
\(R_{\mathrm{SEU}} = \sigma_{\mathrm{SEU}}\,\phi\, N_{\mathrm{bits}}\),
the throughput tax \(\tau\) is the share of processor cycles reclaimed
by error correction and checkpoint replay, namely the upset rate times
the cycles charged per upset, divided by the cycle rate
\(r_{\mathrm{cyc}}\):
...
upset and \(r_{\mathrm{cyc}}\) is the processor cycle rate, so \(\tau\)
is dimensionless. The tax scales linearly with flux, so it peaks in the
South Atlantic Anomaly and the polar horns and falls to a small floor
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the dose-depth law (8.1) and its time integral (8.2),
  show that the shielding thickness at which the attenuated electron
  term equals a fraction \(\epsilon\) of the proton-and-bremsstrahlung
  floor is the closing-thickness expression (8.3),
  \(x^{*} = \lambda \ln\!\left( D_0 / (\epsilon\, D_\infty) \right)\).
  Then show that adding one further attenuation length \(\lambda\) of
  aluminium beyond \(x^{*}\) reduces the residual electron term by the
  factor \(e^{-1}\), and explain in one or two sentences why this is the
  quantitative statement of the chapter's claim that mass past a
  centimetre or two buys almost no margin.
\item
  Using the floor estimate (8.4), \(D_\infty \approx 212\) rad(Si)
  yr\(^{-1}\), and the five-year cumulative dose of approximately 1.06
  krad(Si) at \(x = 10\) mm from (8.2), estimate the
  electron-attenuation prefactor \(D_0\) implied by the chapter's
  calibration with \(\lambda \approx 1.7\) mm. Treat the 1.06 krad(Si)
  as the full five-year dose and the floor as \(212\) rad(Si)
  yr\(^{-1}\) acting over the same five years, and solve (8.2) for
  \(D_0\). Comment on the sign and magnitude of the residual electron
  term at 10 mm relative to the floor.
\item
  The reference mission's high-bandwidth memory first showed
  irregularities at 2 krad(Si) and exhibited no hard failures up to 15
  krad(Si). Using the cumulative-dose law (8.2) with the constants
  determined in Problem 2 (or, equivalently, with the floor-dominated
  approximation \(D(x,T) \approx D_\infty T\) valid for \(x \ge 10\)
  mm), estimate the mission lifetime at \(x = 10\) mm of aluminium that
  would be required to reach each of these two thresholds. By what
  factor do the two lifetimes differ, and how does that factor compare
  with the chapter's stated order-of-magnitude margin to the
  no-hard-failure level?
\item
  The throughput tax is given by (8.5),
  \(\tau = c_{\mathrm{ECC}}\, \sigma_{\mathrm{SEU}}\, \phi\, N_{\mathrm{bits}} / r_{\mathrm{cyc}}\).
  Treating \(c_{\mathrm{ECC}}\), \(\sigma_{\mathrm{SEU}}\),
  \(N_{\mathrm{bits}}\), and \(r_{\mathrm{cyc}}\) as fixed by the
  hardware, show that \(\tau\) scales linearly with the particle flux
  \(\phi\). The chapter states that the tax peaks in the South Atlantic
  Anomaly and the polar horns and falls to a small floor elsewhere,
  consistent with a bounded ten to twenty percent figure. If the
  orbit-averaged tax is to be held at the lower end of that band when
  the flux inside the anomaly is some multiple of the quiescent flux, by
  what factor must the quiescent-region flux be smaller than the anomaly
  flux for the time-averaged tax to remain within the bounded band,
  expressed symbolically in terms of the fraction of orbital time spent
  in each region.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The formation paradox: one fragmentation sprays debris into the volume the neighbours occupy, with the neighbour-hit probability scaling as one over spacing squared.}]{schematics/diagram_formation.png}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the binomial zero-impact probability in Equation (9.2),
  show that in the limit of infinitely many short slices at fixed
  product \(mp = \Phi A N t\) the cluster catastrophic probability
  reduces to Equation (9.1). State precisely where the chapter's
  independence assumption enters, and use Equation (9.5) to argue that
  the same exponent \(N\lambda_1\) is obtained whether one groups the
  fleet into a single Poisson process or treats each satellite as an
  independent survivor.
\item
  Using the per-satellite exposure \(\lambda_1 = \Phi A t\) defined just
  above Equation (9.5) and the chapter's reference values
  \(\Phi = 3\times10^{-5}\ \mathrm{m^{-2}yr^{-1}}\),
  \(A = 15\ \mathrm{m^2}\), and \(t = 5\ \mathrm{yr}\), estimate the
  catastrophic-impact probability for a single satellite over the
  mission. Then form the fleet mean for \(N = 81\) from Equation (9.3)
  and confirm it matches the worked value in Equation (9.4). {[}expect
  \(\Lambda\) of order \(10^{-1}\){]}
\item
  The chapter quotes a flux model band corresponding to a probability
  range from six to forty-six percent about the nominal seventeen
  percent. Treating the nominal mean as \(\Lambda = 0.182\) from
  Equation (9.4) and holding \(A\), \(N\), and \(t\) fixed, determine
  the two flux values \(\Phi\) that reproduce the lower and upper ends
  of that probability band using Equation (9.1). Report each as a
  multiple of the nominal flux.
\item
  A revised design halves the tumbling-averaged silhouette to
  \(A = 7.5\ \mathrm{m^2}\) while leaving \(\Phi\), \(N\), and \(t\)
  unchanged. Using the linearity of Equation (9.3), find the new mean
  \(\Lambda\) and, using Equation (9.1), the new cluster probability
  \(P(K\ge 1)\). State the factor by which the mean changes, and explain
  with reference to the design implication stated after Equation (9.5)
  why this is equivalent to halving either the node count or the mission
  duration.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The passive conduction path from the junction to the radiator, dominated by the junction-to-case resistance.}]{schematics/diagram_thermal.png}}
...
\(A_{\mathrm{rad}} = 1508\ \mathrm{m^2}\). This is of the same order as
the industry rule of roughly 1,200 m² per megawatt and explains why
Starcloud's gigawatt vision implies multi-kilometer panels.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 10.1 Radiator equilibrium temperature versus heat flux, and the radiator area required for one megawatt versus operating temperature. Running hotter reduces area at the cost of silicon lifetime.}]{figures2/figA3_radiator.png}}
...
\(A = 4\ \mathrm{m^2}\) these are about 71 W and 247 W respectively,
which the design lumps to roughly 100 W of parasitic load\index{parasitic load}.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 10.2 Earth view factor versus altitude, and the combined albedo, Earth-infrared, and solar load on a four-square-meter surface. External loads set the parasitic fraction in the radiator sizing.}]{figures2/figA2_loads.png}}
\caption{Fig. 10.2 Earth view factor versus altitude, and the combined
albedo\index{albedo}, Earth-infrared, and solar load on a four-square-meter surface.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 10.1, (a) Radiator area is linear in heat load; (b) the passive thermal chain saturates above \textasciitilde400 W, forcing chip-level active cooling.}]{figures/fig2_thermal_wall.png}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  The chapter states that the radiator area is linear in heat load, with
  no economy of scale. Starting from the steady-state panel energy
  balance (10.4), derive the sizing relation (10.3) explicitly, stating
  where the cosmic-microwave-background sink term is dropped and
  justifying that step with the chapter's estimate of
  \(T_{\mathrm{CMB}}^4/T^4\). Then show that the area per unit power,
  \(A_{\mathrm{rad}}/Q\), is independent of \(Q\), and explain in one
  sentence why this rules out a per-watt cooling advantage for a larger
  plant.
\item
  Using (10.3) and the worked example reference values
  (\(\varepsilon = 0.90\), \(n_s = 2\), \(f_p = 0.12\), operating
  temperature \(T = 20\ \mathrm{C}\), with \(\sigma\) defined in
  (10.2)), estimate the radiator area required to reject a heat load of
  \(Q = 2.5\ \mathrm{MW}\). Report the per-side flux
  \(\varepsilon\sigma T^4\) and the net flux
  \(\varepsilon\sigma T^4 n_s (1 - f_p)\) that you use, and compare your
  result to the industry rule of roughly 1,200 m² per megawatt quoted in
  the chapter. {[}expect order \(10^3\ \mathrm{m^2}\){]}
\item
  The chapter reports that running the silicon hotter buys back radiator
  area through the \(T^4\) dependence, formalized by the sensitivity
  relation (10.5). Using (10.5), estimate the fractional change in
  required area if the operating temperature is raised from
  \(20\ \mathrm{C}\) to \(60\ \mathrm{C}\), working in absolute
  temperature. Then, to test the accuracy of the linearized estimate,
  recompute the area ratio directly from (10.3) at the two temperatures
  and state by what factor the area changes. Comment on whether the
  linear approximation in (10.5) over- or under-estimates the saving
  across this finite temperature step.
\item
  The passive thermal chain imposes a second limit independent of
  radiator size. Using the conductive chain model
  \(T_j = T_{\mathrm{rad}} + Q\,R_{th}\) and the passive ceiling (10.6)
  with the chapter's junction budget
  \(\Delta T_{\max} = 125\ \mathrm{C}\), determine the maximum passively
  coolable chip power for the baseline chain
  \(R_{th} = 0.30\ \mathrm{K/W}\), and confirm it matches the threshold
  in (10.7). Then size the interface: by what factor must \(R_{th}\) be
  reduced so that a single device dissipating \(1200\ \mathrm{W}\) can
  be held within the same junction budget, and state the corresponding
  target value of \(R_{th}\).
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Power flow on the dawn-dusk orbit: the array feeds the bus and loads, and because the orbit is sunlit for more than ninety-five percent of each revolution the battery is barely exercised.}]{schematics/sch_power_budget.png}}
\caption{Power flow on the dawn-dusk orbit: the array feeds the bus and
loads, and because the orbit is sunlit for more than ninety-five percent
of each revolution the battery is barely exercised.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 11.1, Array area versus load, and triple-junction degradation over the mission.}]{systems_figs/fig_power.png}}
...
an implied annual rate \(d = 1 - (0.275/0.296)^{1/5} pprox 0.0146\),
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the multiplicative radiation-degradation model (11.2),
  show that the implied annual degradation rate is
  \(d = 1 - (\eta_{\mathrm{life}}/\eta_{\mathrm{BOL}})^{1/N}\), and that
  for small \(d\) the five-year end-of-life efficiency is approximately
  \(\eta_{\mathrm{life}} \approx \eta_{\mathrm{BOL}}(1 - Nd)\). Using
  \(\eta_{\mathrm{BOL}} = 0.296\), \(\eta_{\mathrm{life}} = 0.275\), and
  \(N = 5\), evaluate the exact rate from (11.2) and compare it with the
  linearized estimate, commenting on the size of the discrepancy.
\item
  Using the load relation (11.1) with the chapter's reference values
  (\(S_0 = 1361\ \mathrm{W/m^2}\), \(\kappa_{\mathrm{pack}} = 0.85\),
  \(\eta_{\mathrm{path}} = 0.88\), \(k_T = -0.0023\ \mathrm{per\ C}\),
  \(\eta_{\mathrm{life}} = 0.275\)), estimate the power delivered to the
  load per square metre of array at a beginning-of-life operating
  temperature of 28 C. Then repeat at the 70 C end-of-life point and
  report the ratio of the two delivered-power densities. {[}expect the
  70 C value to be near the \(253\ \mathrm{W/m^2}\) of the worked
  example{]}
\item
  Size the array for a \(5\ \mathrm{MW}\) load using the sizing closure
  (11.6) at the end-of-life delivered density of
  \(253\ \mathrm{W/m^2}\). Then, treating the area as inversely
  proportional to \(\eta(T,\mathrm{life})\) through (11.6) and (11.3),
  determine by what factor the required area changes if the panel can be
  operated at 40 C instead of 70 C, with all other quantities held
  fixed. {[}expect order \(10^4\ \mathrm{m^2}\) for the baseline area{]}
\item
  Using the radiative-balance result (11.5) with \(\alpha = 0.90\),
  \(\varepsilon_f + \varepsilon_b = 1.6\),
  \(\sigma = 5.67\times 10^{-8}\ \mathrm{W\,m^{-2}\,K^{-4}}\),
  \(S_0 = 1361\ \mathrm{W/m^2}\), and \(\kappa_{\mathrm{pack}} = 0.85\),
  carry out one fixed-point iteration for the equilibrium temperature:
  begin from the 70 C operating point, evaluate \(\eta\) from (11.1) at
  that temperature, substitute into (11.5) to obtain an updated \(T\),
  and then re-evaluate \(\eta\) at the updated temperature. Report the
  temperature after the iteration and state whether a hotter panel
  raises or lowers the equilibrium temperature, consistent with the
  coupling noted after (11.5).
\end{enumerate}
...
analyses. The reference four-chip satellite closes at roughly 220 kg
dry.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 12.1 Reference satellite dry-mass breakdown by subsystem, closed self-consistently by the integrated model.}]{figures2/figA8_mass.png}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the volume integral (12.2), show that the principal
  moment about the \(y\) axis of a uniform rectangular solid is
  \(I_y = \tfrac{m}{12}(d_x^2 + d_z^2)\). Identify the squared distance
  from the \(y\) axis that replaces \(y^2 + z^2\), carry out the three
  integrations using \(\int_{-d/2}^{d/2} s^2\,ds = d^3/12\), and confirm
  that your result is the cyclic permutation of (12.3). Then add the
  three principal moments and state the form of \(I_x + I_y + I_z\) in
  terms of \(d_x, d_y, d_z\).
\item
  A deployed solar wing of mass \(m = 30\ \mathrm{kg}\) and length
  \(L = 6\ \mathrm{m}\) is mounted with its centroid at an offset
  \(o = 4\ \mathrm{m}\) from the bus axis. Using the parallel-axis
  relation (12.4), evaluate the moment of inertia this single wing
  contributes about the bus axis. Then report separately the centroidal
  term \(mL^2/12\) and the offset term \(mo^2\), and state which
  dominates. Compare the total with the chapter's statement that the
  maximum principal moment of the reference satellite is on the order of
  a few hundred kilogram-meters squared. {[}expect the offset term to
  dominate{]}
\item
  Using the minimum-area relation (12.6), size a single primary member.
  Take a tributary mass \(m = 80\ \mathrm{kg}\), the chapter's
  quasi-static load factor \(g_{\mathrm{load}} = 8.5\), standard gravity
  \(g_0 = 9.81\ \mathrm{m\,s^{-2}}\), an aluminium yield strength
  \(\sigma_y = 270\ \mathrm{MPa}\), and a factor of safety
  \(F_s = 1.25\). Compute the minimum cross-sectional area
  \(A_{\mathrm{member,\,min}}\) in square millimeters. Then, using
  (12.5), state by what factor the strength margin \(\mathrm{MS}\) above
  zero increases if the actual member area is doubled relative to
  \(A_{\mathrm{member,\,min}}\).
\item
  The chapter's design implication states that inertia grows with the
  square of panel length while the launch-driven member area grows only
  linearly with mass. Treat a deployed panel as a uniform plate whose
  mass is proportional to its area, so that for fixed thickness and
  offset the mass scales as \(m \propto L^2\). Using (12.4) with the
  offset term neglected, and (12.6), determine the factor by which the
  centroidal inertia contribution \(mL^2/12\) changes and the factor by
  which the required member area \(A_{\mathrm{member,\,min}}\) changes
  when the panel length \(L\) is increased by 50 percent. Use the
  comparison to justify the chapter's recommendation that the mass
  budget be tightened at the geometry stage rather than at member
  sizing.
\end{enumerate}
...
fast-steering mirror\index{fast-steering mirror} is required on top of the spacecraft bus.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Coarse body pointing combined with a fast-steering mirror, which supplies the sub-microradian correction the optical link requires.}]{schematics/diagram_pointing.png}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the general gravity-gradient torque (13.2),
  \(\boldsymbol{\tau}_{\mathrm{gg}} = 3 n^2\, \hat{\mathbf{r}} \times (\mathbf{I}\,\hat{\mathbf{r}})\),
  resolve the local vertical for a single-axis pitch offset \(\theta\)
  as \(\hat{\mathbf{r}} = (\sin\theta,\, 0,\, \cos\theta)\) in the
  principal frame and show that the only surviving torque component
  reduces to (13.3),
  \(\tau_{\mathrm{gg}} = \tfrac{3}{2}\, n^2\, (I_z - I_x)\, \sin 2\theta\).
  State the two pitch angles in \([0^\circ, 90^\circ]\) at which the
  torque vanishes and the angle at which it is maximum, and explain why
  this result motivates flying the bus with its minimum-inertia axis
  along the local vertical.
\item
  Using the mean motion at the reference altitude from (13.4),
  \(n \approx 1.07\times10^{-3}\ \mathrm{rad\,s^{-1}}\), and the peak
  gravity-gradient torque from (13.3), estimate the maximum disturbance
  torque (\(\theta = 45^\circ\)) for an inertia asymmetry
  \(|I_z - I_x| = 100\ \mathrm{kg\,m^2}\). Confirm that the result is
  consistent with the order \(10^{-4}\ \mathrm{N\,m}\) quoted in the
  chapter. {[}expect order \(10^{-4}\ \mathrm{N\,m}\){]}
\item
  The pointing-loss factor is
  \(L_{\mathrm{point}} = \exp(-2\theta_{\mathrm{err}}^2/\theta_{\mathrm{div}}^2)\),
  and its small-jitter linearization (13.5) gives
  \(\theta_{\mathrm{err}} \le \theta_{\mathrm{div}}\sqrt{\epsilon/2}\)
  for a fractional power-loss budget \(\epsilon\). For the
  diffraction-limited divergence \(\Theta \approx 2.44\,\lambda/D\),
  take the full divergence as \(\theta_{\mathrm{div}} = \Theta\) and
  size the maximum allowable residual jitter \(\theta_{\mathrm{err}}\)
  for a link operating at \(\lambda = 1.55\ \mu\mathrm{m}\) through an
  aperture \(D = 0.10\ \mathrm{m}\), holding the pointing loss to
  \(\epsilon = 0.10\). Compare your result to the two-arcsecond (roughly
  ten-microradian) attitude knowledge a star tracker provides, and state
  whether the bus alone can meet the requirement.
\item
  Two laser links are to be assessed with the same transmit aperture
  \(D = 0.10\ \mathrm{m}\) and wavelength
  \(\lambda = 1.55\ \mu\mathrm{m}\): a 150-m neighbor hop and a long
  inter-satellite hop. Using the divergence relation
  \(\Theta \approx 2.44\,\lambda/D\), compute the beam footprint
  (diameter) at 150 m and confirm the chapter's claim that the neighbor
  link is geometrically trivial because the spot is millimeters wide.
  Then, using the linearized requirement
  \(\theta_{\mathrm{err}} \le \theta_{\mathrm{div}}\sqrt{\epsilon/2}\)
  from (13.5), state by what factor the residual-jitter requirement
  tightens if a redesign doubles the aperture to
  \(D = 0.20\ \mathrm{m}\) at fixed \(\epsilon\), and explain why
  tighter beams force reliance on a fine-steering mirror slaved to the
  received beacon.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Optical-beam divergence: the fraction of light captured by the receiver falls as one over range squared, which forces close spacing.}]{schematics/diagram_link.png}}
...
\textbf{Why the formation is tight.} A representative 1550 nm
inter-satellite terminal (an 80-mm receive telescope, 15-microradian
divergence, 1-microradian pointing, 10-Gbps on-off keying) at one watt
with three-decibel margin gives a reliable range of order 5,000 km; the
analysis anchors to the published design point of about 5,419 km from
the free-space optical satellite-link study of Liang et al.~{[}6{]}.
Because free-space path loss\index{free-space path loss} rises twenty decibels per decade of range,
the required power scales as the inverse square, from 34 mW at 1,000 km
to 3.41 W at 10,000 km, against a commercial terminal limit near one
watt. The reference architecture needs roughly ten terabits per second
of aggregate bandwidth per link, achievable with off-the-shelf
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 14.2 Transmit power required to hold a three-decibel link margin versus range, against the one-watt terminal limit, and the pointing-loss sensitivity to jitter. Range and pointing together force the tight formation.}]{figures2/figA4_link.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 14.1, Inter-satellite free-space loss versus range and pointing-loss sensitivity.}]{systems_figs/fig_comms.png}}
...
decade while pointing loss\index{pointing loss} grows quadratically with jitter, link closure
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the spreading argument that leads to Eq. (14.2), show
  that the product of transmit and receive telescope gains for two
  identical circular apertures of diameter \(D\), combined with the
  free-space loss of Eq. (14.1), gives a net range dependence
  \(G_{\mathrm{tx}}G_{\mathrm{rx}}/L_{\mathrm{fs}} \propto (D^2/(\lambda R))^2\).
  Use this to explain, in terms of the exponents on \(D\), \(\lambda\),
  and \(R\), why doubling each aperture diameter buys the same
  received-power improvement as halving the range.
\item
  Using the free-space-loss expression of Eq. (14.3), evaluate
  \(L_{\mathrm{fs}}\) in decibels at the chapter's reference wavelength
  of 1550 nm for the two ranges named in the text, 1,000 km and 10,000
  km. Confirm from your two values that the difference matches the
  twenty-decibel-per-decade slope stated in Eq. (14.3), and comment on
  how this slope is consistent with the quoted jump from 34 mW to 3.41 W
  in required transmit power across the same decade of range.
\item
  The pointing loss of Eq. (14.4) is evaluated in the chapter at
  \(\theta_e = 1\ \mu\mathrm{rad}\) against a divergence of
  \(\theta_{\mathrm{div}} = 15\ \mu\mathrm{rad}\), giving about 0.04 dB.
  By what factor does the pointing loss in decibels change if pointing
  jitter degrades so that \(\theta_e\) rises to \(5\ \mu\mathrm{rad}\)
  while \(\theta_{\mathrm{div}}\) is held fixed? Using the quadratic
  form of Eq. (14.4), state the general rule for how the decibel
  pointing loss scales with \(\theta_e\), and use it to explain the
  chapter's statement that jitter approaching the divergence angle
  degrades the budget sharply.
\item
  Size the ground segment for cloud-cover availability. Using the
  site-diversity relation \(1 - (1 - p_{\mathrm{clear}})^n\) from the
  chapter, and the chapter's statement that four stations reach roughly
  ninety-six percent, first infer the single-site clear-sky probability
  \(p_{\mathrm{clear}}\) consistent with that figure. Then determine the
  smallest number of independent stations \(n\) needed to raise the
  at-least-one-clear probability above ninety-nine percent. Separately,
  using the latency-crossover result of the chapter, state the fixed
  fractional time advantage of the vacuum mesh over fiber that follows
  from \(n_{\mathrm{fiber}} = 1.4675\), and confirm it is the
  forty-seven percent quoted in the latency-crossover discussion.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Controlled de-orbit: a retrograde burn drops the far side of the orbit onto an atmosphere-grazing transfer ellipse.}]{schematics/diagram_hohmann.png}}
...
Combined with the twelve-year natural lifetime of Chapter 7, this makes
active de-orbit mandatory.
...
dominates the five-year budget of about 202 m/s (drag make-up
\textasciitilde24, formation \textasciitilde8, collision avoidance
...
\(m_p = m_{\mathrm{dry}}(e^{\Delta v/(I_{sp}g_0)} - 1)\). For 202 m/s on
a 220-kg dry bus, this is about 22 kg with hydrazine or about 3 kg with
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 15.1, Per-satellite Δv budget (de-orbit dominates) and propellant mass by propulsion type.}]{figures/fig5_deltav.png}}
...
the Tsiolkovsky rocket equation\index{rocket equation}. Writing the mass ratio as
...
impulse is decisive: for the total budget of 202 m/s on the 220-kg dry
bus, a hydrazine system at modest \(I_{sp}\) needs about 22 kg of
...
requirement falls to roughly 3 kg.
...
architecture; selecting high specific impulse\index{specific impulse} minimizes wet mass at the
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the vis-viva equation (15.2), show that the speed at the
  apogee of the de-orbit transfer ellipse can be written as
  \(v_{\mathrm{apo}} = v_{c1}\sqrt{2 r_2/(r_1 + r_2)}\), where
  \(v_{c1} = \sqrt{\mu/r_1}\) is the circular speed at the initial
  radius and \(a_t = (r_1 + r_2)/2\) is the transfer semi-major axis.
  Then take the limit \(r_2 \to r_1\) and interpret the resulting value
  of \(\Delta v_{\mathrm{deorbit}}\) from (15.1) physically.
\item
  Using the de-orbit relation (15.1) and the worked-example reference
  values \(r_1 = 7.021\times10^6\) m and \(v_{c1} = 7534.8\) m/s,
  compute the de-orbit impulse required to lower the perigee instead to
  \(120\) km, for which \(r_2 = 6.491\times10^6\) m. Compare your result
  to the chapter value of \(131.6\) m/s for a \(180\)-km perigee, and
  state by what factor the de-orbit term changes.
\item
  Using the propellant sizing relation (15.5) with the chapter's total
  budget \(\Delta v = 202\) m/s and dry mass \(m_{\mathrm{dry}} = 220\)
  kg, size the propellant mass \(m_p\) for a hydrazine system whose
  specific impulse is \(I_{sp} = 220\) s, taking \(g_0 = 9.81\)
  m/s\(^2\). Then evaluate the small-argument approximation
  \(m_p \approx m_{\mathrm{dry}}\,\Delta v/(I_{sp} g_0)\) quoted in the
  chapter and report the fractional error it introduces at this
  \(I_{sp}\). {[}expect order \(20\,\)kg{]}
\item
  The chapter notes that propellant scales exponentially with
  \(\Delta v\) through (15.5), while the de-orbit term dominates the
  budget. Treating \(m_{\mathrm{dry}}\), \(I_{sp}\), and \(g_0\) as
  fixed, derive an expression for the sensitivity \(dm_p/d(\Delta v)\)
  and use it to estimate the additional propellant mass incurred by a
  \(20\) m/s increase in the velocity budget for the hydrazine system of
  Problem 3. Comment on whether this sensitivity argument strengthens
  the chapter's design implication that disposal, not station-keeping,
  sets the propulsion architecture.
\end{enumerate}
...
availability: exponential survival, k-of-n\index{k-of-n} redundancy, and the
continuous replenishment needed to hold capacity over the mission.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Redundancy and replenishment: the constellation deploys n units and requires k of them, while continuous resupply at about one over the design life per year holds capacity.}]{schematics/sch_redundancy.png}}
\caption{Redundancy and replenishment: the constellation deploys n units
and requires k of them, while continuous resupply at about one over the
design life per year holds capacity.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 16.1, Per-node reliability versus time and the over-provisioning needed to hold capacity.}]{systems_figs/fig_reliability.png}}
...
count then follows from inverting the availability constraint. A
mean-capacity lower bound, holding the expected number of survivors at
\(n_{\mathrm{required}}\) for per-unit reliability \(p\) and
availability target \(A\), is

\begin{equation}
n_{\mathrm{deployed}} \gtrsim \frac{A\,n_{\mathrm{required}}}{p}, \tag{16.7}
\end{equation}

which for the figures above gives about 80. The stricter requirement,
that the probability of at least \(n_{\mathrm{required}}\) survivors
itself reach \(A\) evaluated from the binomial tail (16.2), is what
\passthrough{\lstinline!spares\_for\_availability!} applies and raises
the figure to about 89, the value quoted earlier. The design implication
is that survival reliability, redundancy margin, and resupply cadence
are not independent choices; fixing any two through Eqs. (16.6) and
(16.7) determines the third and, with it, the recurring launch and
manufacturing budget carried in the mission economics.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the constant-hazard reliability of Eq. (16.1), show that
  the median time to failure of a unit is
  \(t_{1/2} = \mathrm{MTBF}\,\ln 2\), and confirm using (16.4) that this
  median is strictly smaller than the mean lifetime. State in one
  sentence why a constant hazard produces a median that lies below the
  mean.
\item
  Using Eq. (16.1) and the bus figures in the chapter, estimate the
  five-year survival probability of a unit whose mean time between
  failures is 100,000 hours. Then, using the mean-capacity bound (16.7),
  estimate the deployed count required to hold an availability of
  \(A = 0.95\) for \(n_{\mathrm{required}} = 75\) nodes at this per-unit
  reliability. {[}expect a deployed count on the order of 100 units{]}
\item
  Using the renewal balance of Eq. (16.6), size the steady-state annual
  replacement count for a constellation of 120 deployed units at a
  design life of \(L = 7\) years, and express the result as a percentage
  of the fleet per year. By what factor does this annual replacement
  count change if the design life is extended from five years to ten
  years at fixed fleet size?
\item
  The chapter notes that the expected number of survivors is
  \(\mathbb{E}[X] = np\) with variance \(np(1-p)\) from Eq. (16.5). For
  \(n = 81\) deployed units at \(p = 0.90\), compute \(\mathbb{E}[X]\)
  and the standard deviation, and determine how many standard deviations
  the required count \(k = 70\) sits below the mean. Using this margin,
  comment on why raising the per-unit reliability from \(p = 0.90\) to
  \(p = 0.95\) produces the large gain in the tail sum (16.2) reported
  in the chapter.
\end{enumerate}
...
compute that is actually delivered, after utilization\index{utilization}, the radiation
tax, and thermal throttling are accounted for.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Delivered compute as a fraction of the headline peak, after real utilization and the radiation tax.}]{schematics/diagram_compute.png}}
...
low-power TPU throttles only about fourteen percent. This is the
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 17.1, Added subsystems: passive thermal-throttle by chip, ground-station site diversity, latency crossover, and the discounted-cash-flow breakeven.}]{systems_figs/fig_new_subsystems.png}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  The chapter writes the model-FLOPs utilization and radiation
  availability as \(\mathrm{MFU} = 1 - \ell_u\) and \(a_r = 1 - \ell_r\)
  and states in (17.2) that the throughput penalties add to first order.
  Starting from the exact delivered rate (17.1), expand the product
  \((1 - \ell_u)(1 - \ell_r)\) and show that the leading correction
  dropped in passing to (17.2) is the single term \(\ell_u \ell_r\).
  Then derive a general expression for the fractional error of the
  additive approximation (17.2) relative to the exact (17.1), and state
  in one sentence why the approximation is most trustworthy when one
  loss fraction dominates the other.
\item
  Using the worked-example reference values of the chapter,
  \(\mathrm{MFU} = 0.10\) for inference decode and \(a_r = 0.95\), apply
  (17.1) to report the delivered fraction of installed peak. Then apply
  (17.7) to state the factor by which the unit cost of useful work is
  inflated relative to a peak-rated quote, and apply (17.6) to convert a
  compute-direct overhead ratio of \(1.2\) into an effective power-usage
  effectiveness. {[}expect a delivered fraction near 0.095 and a cost
  inflation near a factor of 10{]}
\item
  Consider the throttled fixed point (17.5). Two accelerators share the
  same throttle gain \(\beta\), the same junction-to-radiator resistance
  \(R_{\theta}\), the same requested utilization \(u_0\), and operate
  over the same radiator temperature \(T_{\mathrm{rad}}\), but one is
  the 700-W part and the other is a low-power part as contrasted in the
  chapter. Using (17.5), show that the throttled effective utilization
  depends on the part only through the product
  \(\beta R_{\theta} P_{\mathrm{peak}}\) in the denominator. By what
  factor does the denominator of (17.5) change if \(P_{\mathrm{peak}}\)
  is reduced by a factor of seven while \(\beta\), \(R_{\theta}\), and
  \(u_0\) are held fixed, and what does this imply for the effective
  utilization of the lower-power part relative to the high-power part?
\item
  The chapter states that chip-level active cooling acts on (17.5) by
  lowering the junction-to-radiator thermal resistance \(R_{\theta}\).
  For a fixed high-power accelerator with given \(u_0\), \(\beta\),
  \(T_{\mathrm{rad}}\), and \(P_{\mathrm{peak}}\) in the throttling
  regime, derive from (17.5) the value of \(R_{\theta}\) required to
  recover a target throttled utilization \(u_{\mathrm{eff}}^{\star}\),
  expressed in terms of the chapter's symbols. Then, using your result,
  explain why design effort spent lowering \(R_{\theta}\) returns more
  delivered work for a high-\(P_{\mathrm{peak}}\) part than for a
  low-\(P_{\mathrm{peak}}\) part, and connect this to the design
  implication that utilization is the largest single factor for an
  inference-dominated orbital data center.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The integrated design loop: power, array area, mass, propulsion, and thermal are solved together and iterated to a self-consistent design point.}]{schematics/sch_budget_closure.png}}
\caption{The integrated design loop: power, array area, mass,
propulsion, and thermal are solved together and iterated to a
self-consistent design point.}
\end{figure}
...
total electrical power. That power sizes the solar array\index{solar array} area and mass
...
levelized cost\index{levelized cost} of compute.
...
power, and shielding dominate, and the design closes at roughly 220 kg
dry.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 18.1, Integrated dry-mass budget, closed self-consistently by the system model.}]{systems_figs/fig_mass_budget.png}}
...
\(m_0 \approx 176\) kg closes at \(m_{\mathrm{dry}} \approx 220\) kg for
the reference four-chip satellite.
...
\def\labelenumi{\arabic{enumi}.}
\item
  The mass fixed point of (18.3) collects every kilogram fixed by
  physics into \(m_0\) and lets the structure scale as
  \(f_{\mathrm{str}}\,m_{\mathrm{dry}}\). Starting from (18.3), show
  that the closed form (18.4) follows, and then show that the marginal
  sensitivity of the dry mass to the payload-and-bus term is
  \(\partial m_{\mathrm{dry}} / \partial m_0 = 1/(1 - f_{\mathrm{str}})\),
  so that the structural amplifier acts identically on the closed mass
  and on its derivative. Comment on the behavior of both quantities as
  \(f_{\mathrm{str}} \to 1\) and relate this to the design implication
  stated in the chapter.
\item
  Using the worked example values of the chapter, namely a
  payload-and-bus term \(m_0 \approx 224\) kg and a structural fraction
  \(f_{\mathrm{str}} = 0.20\), evaluate the structural amplification
  \(1/(1 - f_{\mathrm{str}})\) of (18.4) and the resulting dry mass
  \(m_{\mathrm{dry}}\). Confirm that the result lies inside the
  reference four-chip range stated in the chapter. {[}expect
  \(m_{\mathrm{dry}}\) of order \(280\) kg{]}
\item
  The chapter notes that the structural and contingency fractions
  multiply the entire budget, so savings there propagate through the
  whole design. Using (18.4), determine by what factor the dry mass
  changes if the structural fraction is reduced from
  \(f_{\mathrm{str}} = 0.20\) to \(f_{\mathrm{str}} = 0.12\), holding
  the payload-and-bus term \(m_0\) fixed. Express the answer as the
  ratio of the two amplification factors, and state in words why
  trimming a fraction is more effective than trimming any single
  subsystem mass inside \(m_0\).
\item
  Once the dry mass is fixed, the wet mass follows from the propulsive
  amplifier of (18.6),
  \(m_{\mathrm{wet}} = m_{\mathrm{dry}}\exp\!\left(\Delta v / (I_{sp}\,g_0)\right)\).
  Treating \(m_{\mathrm{dry}}\) as the closed value from (18.4), show
  that the wet mass can be written as a product of the two amplifiers
  acting on the payload-and-bus term,
  \[ m_{\mathrm{wet}} = \frac{m_0}{1 - f_{\mathrm{str}}}\,\exp\!\left(\frac{\Delta v}{I_{sp}\,g_0}\right), \]
  and identify which factor is the structural amplification and which is
  the propulsive amplification. Then explain, using (18.5), why a higher
  dry mass at fixed drag area \(A_d\) and drag coefficient \(C_D\)
  raises the ballistic coefficient \(\beta\) and therefore slows the
  natural decay.
\end{enumerate}
...
quartic radiator energy balance is solved by Newton-Raphson\index{Newton-Raphson}. The
...
launch-cost-parity year is obtained by Monte-Carlo\index{Monte-Carlo} over the uncertain
...
and the fourth-order Runge-Kutta integrator, used for orbital decay\index{orbital decay} and
...
lets the 12.2 yr decay lifetime and the small eclipse temperature drop
...
in about two and a half years at solar maximum lasts roughly one hundred
seventy years at solar minimum, which is precisely why the design cannot
rely on passive disposal. Second, delivered compute is sensitive to
model-FLOPs utilization and to the radiation throughput tax, which is
why the realistic cost of compute is several times the naive peak-based
figure. The single weakest input is the chip power itself, undisclosed
by the vendor and treated as a 150-to-200-watt band.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 19.1, Monte-Carlo distributions of natural lifetime, debris probability, and total Δv across the dominant uncertainties.}]{systems_figs/fig_montecarlo.png}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the first-order Taylor expansion (19.2) and the general
  correlated variance result (19.3), show that for a pure power-law
  monomial \(y = c\,\prod_{j} x_j^{a_j}\) with independent inputs the
  linear-propagation formula (19.1) reduces to the relative-error rule
  (19.4). In your derivation, evaluate the partial derivative
  \(\partial y/\partial x_j\) explicitly and state where the
  independence assumption enters.
\item
  Using the relative-error rule (19.4) and the chip-power band of 150 to
  200 watts, compute the symmetric fractional spread of chip power about
  its midpoint. Then, treating the catastrophic-impact probability as a
  product of debris flux and exposed area over the mission with all
  exponents equal to unity, estimate the output fractional uncertainty
  if a single additional independent input carries a fractional spread
  of 0.10, before the lognormal density term is added by sampling.
\item
  The chapter states that the same satellite decays in about two and a
  half years at solar maximum and lasts roughly one hundred seventy
  years at solar minimum, with atmospheric density moving by up to a
  factor of one hundred across the solar cycle. Using these reference
  values, estimate the local sensitivity index defined in the chapter as
  the ratio of the output swing to the baseline, taking natural lifetime
  as the output and the solar-maximum value as the baseline. Comment on
  why linear error propagation (19.1) is inadequate for this input and
  why the chapter supplements it with Monte-Carlo sampling.
\item
  Consider an output formed as a product of three independent inputs,
  all entering with exponent unity, where two of the inputs carry a
  fractional spread of 0.143 each and the third is a well-known
  parameter with a fractional spread of 0.02. Using (19.4), determine
  the output fractional uncertainty, and then determine by what factor
  the output fractional uncertainty changes if the third input is
  refined to a fractional spread of zero. Use the result to justify the
  chapter's design implication that the output band is set almost
  entirely by the one or two widest inputs.
\end{enumerate}
...
Natural lifetime (moderate solar) & \textasciitilde12 yr & Ch. 7, fails
5-yr rule \\
...
Reference dry mass & \textasciitilde220 kg & Ch. 12, 18 \\
\end{longtable}
}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 20.1, Catastrophic debris risk for the full cluster, and the in-cluster cascade coupling versus formation spacing.}]{figures/fig3_debris_risk.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 20.2, Cumulative dose versus aluminium shielding, and the reliability-throughput trade.}]{figures/fig4_radiation.png}}
...
important design variable here. The drag area of a few square metres
over a dry mass of roughly 220 kg, as listed in Table 20.1, sets \(A/m\)
and a natural lifetime near twelve years at moderate solar activity;
this still exceeds the 5-year passive-disposal rule, which is precisely
...
For small \(p_1\) this is close to \(N p_1\), which is an upper bound:
the exact cluster probability \(1 - (1 - p_1)^N\) is concave in \(N\)
and bends below the linear estimate, saturating toward one. The cluster
figure of about 17 percent is therefore dominated by member count, and
adding satellites raises the cluster risk close to linearly at this
scale, with the per-member marginal risk slowly decreasing as \(N\)
grows. The design implication is direct: constellation sizing cannot be
separated from the per-member debris hazard, and growth in \(N\) must be
paid for in either shielding mass or accepted mission risk.
...
operating cost, net present value\index{net present value}, and the levelized cost of useful
compute.
\end{quote}
...
dollars of hardware on a 233-kg satellite,
\(p^{*} \approx 4{,}720\ \mathrm{USD/kg}\). Below this price, hardware
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 21.1b Levelized cost of useful compute versus model-FLOPs utilization, and versus launch price. The cost is dominated by utilization and hardware, not by launch.}]{figures2/figA7_lcoe.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 21.1, Cost of useful compute versus utilization, and the radiation throughput tax by protection level.}]{systems_figs/fig_compute.png}}
...
\(m_{\mathrm{launch}} = 233\ \mathrm{kg}\), the cross-over evaluates to
\(p^{*} = 1.1\times 10^{6}/233 \approx 4{,}720\ \mathrm{USD/kg}\),
...
capital-recovery factor\index{capital-recovery factor} used in (21.1),
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the per-satellite cost split (21.2), show that the
  launch term and the hardware term are equal at the cross-over price
  given by (21.3), and prove that the partial derivative of
  \(C_{\mathrm{sat}}\) with respect to \(p_{\mathrm{launch}}\) equals
  \(m_{\mathrm{launch}}\) and is independent of \(p_{\mathrm{launch}}\).
  Use this constant-slope result to explain why, once
  \(p_{\mathrm{launch}} \ll p^{*}\), further reductions in launch price
  return little, and identify the hardware floor onto which
  \(C_{\mathrm{sat}}\) flattens.
\item
  Using the cross-over relation (21.3) with the chapter's reference
  values \(C_{\mathrm{hw}} \approx 1.1\times 10^{6}\ \mathrm{USD}\) and
  \(m_{\mathrm{launch}} = 233\ \mathrm{kg}\), estimate the cross-over
  price \(p^{*}\). Then, using (21.2), estimate the total per-satellite
  cost \(C_{\mathrm{sat}}\) at a launch price of
  \(p_{\mathrm{launch}} = 600\ \mathrm{USD/kg}\) and report the launch
  term as a fraction of \(C_{\mathrm{sat}}\), confirming the chapter's
  statement that below roughly 600 USD/kg the launch term is a small
  correction. {[}expect \(p^{*}\) of order
  \(10^{3}\ \mathrm{USD/kg}\){]}
\item
  Starting from the discounted present value of \(n\) equal annual
  payments in (21.4), derive the capital-recovery factor (21.5) by
  inverting for the annuity per unit principal. Then show that in the
  limit \(r \to 0\) the factor reduces to \(\mathrm{CRF} \to 1/n\),
  recovering straight-line amortization, and argue that for \(r > 0\)
  the factor exceeds \(1/n\).
\item
  Using the useful-compute chain (21.6) and the proportionality
  \(\mathrm{LCOE} \propto 1/(\eta_{\mathrm{th}}\, u\, A_{\mathrm{op}})\)
  implied by (21.1) and (21.6), determine by what factor the levelized
  cost of compute changes if a chip throttles from
  \(\eta_{\mathrm{th}} = 1.0\) to \(\eta_{\mathrm{th}} = 0.5\) while
  \(u\) and \(A_{\mathrm{op}}\) are held fixed. Then size the throttling
  factor \(\eta_{\mathrm{th}}\) required to offset a drop in model-FLOPs
  utilization from \(u = 0.6\) to \(u = 0.4\) so that the annual useful
  PFLOP-hours of (21.6) are unchanged, holding \(P_{\mathrm{peak}}\) and
  \(A_{\mathrm{op}}\) fixed. {[}expect the cost to double in the first
  part{]}
\end{enumerate}
...
ten percent and moves the conditional median to about 2050, so the
conclusion is not an artifact of where the horizon is drawn. The
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the Wright relation (22.1) and the definition of the
  learning rate as the fractional price reduction per doubling of
  cumulative launched mass, show that the exponent satisfies
  \(b = -\ln(1-L)/\ln 2\) as in (22.2). Then show that the
  cumulative-mass ratio required to reach a target price, equation
  (22.3), can be written as \(m/m_0 = (C_0/C^\ast)^{1/b}\), and explain
  why a learning rate that is smaller than assumed makes this ratio
  larger for a fixed target price.
\item
  Using (22.2), evaluate the learning exponent \(b\) and the inverse
  exponent \(1/b\) at the two ends of the sampled learning-rate range,
  \(L = 0.15\) and \(L = 0.25\). For each case, use (22.3) to find the
  factor by which cumulative launched mass must rise to drive the
  effective price down by a factor of ten toward the 200 dollars per
  kilogram target. Comment on how much the required mass factor changes
  across the sampled learning-rate range. {[}At \(L=0.20\) the chapter
  gives a factor near \(1.3\times 10^3\).{]}
\item
  The chapter's worked estimate takes the effective price to fall by
  roughly a factor of ten to approach the target. Treat the 2026
  effective price as the reference price \(C_0\) in (22.1) and use the
  sampled range of 1,200 to 3,500 dollars per kilogram. For each end of
  that range, compute the actual price ratio \(C_0/C^\ast\) needed to
  reach \(C^\ast = 200\) dollars per kilogram, then use (22.3) with
  \(L = 0.20\) to find the required cumulative-mass ratio \(m/m_0\) at
  each end. Discuss how the choice of starting price alone, holding the
  learning rate fixed, shifts the required cumulative mass.
\item
  Using the required cumulative-mass ratio from the chapter's worked
  estimate (\(m/m_0 \approx 1.3\times 10^3\) at \(L = 0.20\)) together
  with the cadence model (22.4), size the number of flights needed to
  accumulate the required mass. Take a payload of \(p = 200\) tonnes per
  launch, take the reference mass \(m_0\) to be the additional
  cumulative mass already implied near the start of the ramp, and use
  the chapter's stated figure of roughly 370,000 tonnes of additional
  cumulative launched mass to the target. Estimate the number of
  launches this implies at 200 tonnes per flight, and compare your
  figure with the chapter's order of 1,800 Starship launches. Then,
  holding the required mass fixed, state by what factor the number of
  launches changes if the payload is reduced from 250 tonnes to 150
  tonnes, the two ends of the sampled payload range.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Technology-readiness ladder: the components sit mid-scale, while system integration sits near the bottom.}]{schematics/diagram_trl.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Fig. 25.1, Systems-level risk matrix: cooling is solved; debris, disposal, and economics are the open items.}]{figures/fig7_risk_matrix.png}}
...
feeds a natural lifetime of roughly twelve years at moderate solar
activity, which exceeds the five-year rule under nominal conditions and
mandates the 132 m/s de-orbit burn of Table T7. The coupled
thermal-throttle simulation returns the delivered useful compute (about
two-thirds of a petaflop for this low-power, lightly throttled case),
the ground-link model returns tens of terabytes per day of downlink at
ninety-six-percent availability, and the finance model returns a
levelized cost of roughly one hundred dollars per useful petaflop-hour
(Table T11). Aggregating over the eighty-one-satellite cluster gives the
fleet totals of Table T12. Every one of these numbers is produced by the
open code and is reproduced by the validation suite.
...
For the reference point \(T_{\text{nat}}\) is near twelve years at
moderate solar activity while \(T_{\text{rule}} = 5\) yr, so the
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the power-summation rule of the chapter, show that the
  electrical power-usage effectiveness can be written purely in terms of
  the contingency margin, the compute load, and the subsystem sum,
  namely that combining (26.1) and (26.2) gives
  \(\mathrm{PUE}_{\text{elec}} = (1+m)\left(1 + \sum_i P_i / P_{\text{IT}}\right)\).
  Using this form, state by inspection how
  \(\mathrm{PUE}_{\text{elec}}\) responds in the limit where the fixed
  subsystem allocations become negligible compared with the accelerator
  draw, and confirm that the reference design point reproduces the
  tabulated value of 1.58.
\item
  Recompute the total electrical demand using (26.1) for a design point
  in which the number of accelerators is doubled to eight while every
  other input is held at its reference value (200 W per accelerator, the
  same avionics, attitude-control, communications, and thermal-control
  allocations summing to 300 W, and \(m = 0.15\)). Report the new
  \(P_{\text{elec}}\) and the new \(\mathrm{PUE}_{\text{elec}}\) from
  (26.2), and state by what factor the electrical demand increases
  relative to the four-accelerator reference value of 1,265 W. {[}expect
  \(\mathrm{PUE}_{\text{elec}}\) to fall below the reference 1.58{]}
\item
  The disposal decision turns on the ballistic coefficient through
  (26.3) and the lifetime inequality (26.4). For the reference dry mass
  near 220 kg, treat the natural lifetime as scaling inversely with the
  ballistic coefficient as described in the chapter, and determine the
  factor by which \(\beta\) would have to be reduced to bring the
  natural lifetime down to the regulatory limit \(T_{\text{rule}} = 5\)
  yr, taking the natural lifetime near twelve years at moderate solar
  activity. Then, using (26.3), state by what factor the drag area \(A\)
  must increase to achieve that reduction in \(\beta\) at fixed dry mass
  and fixed drag coefficient\index{drag coefficient}, and comment on whether passive area
  increase alone removes the need for the 132 m/s de-orbit burn of
  (26.4).
\item
  Using the levelized-cost definition (26.5), size the sensitivity of
  the figure of merit to the throttled compute. Holding the lifecycle
  cost \(C_{\text{life}}\) and the operating horizon \(T_{\text{op}}\)
  fixed at their reference values, and taking the delivered useful
  compute rate \(Q\) as the roughly two-thirds of a petaflop quoted in
  the chapter against a levelized cost of about one hundred dollars per
  useful petaflop-hour, determine the new levelized cost if thermal
  throttling were relieved enough to raise \(Q\) by twenty percent.
  Compare this with the change produced by a twenty-percent reduction in
  launch price, and use the structure of (26.5) together with the role
  of \(\mathrm{PUE}_{\text{elec}}\) in (26.2) to justify the chapter's
  design implication that reducing parasitic overhead and throttling
  improves the economics more than an equivalent reduction in launch
  price.
\end{enumerate}
...
dB telescope gain\index{telescope gain}, is why the cluster must be tight.
...
formation, the Clohessy-Wiltshire\index{Clohessy-Wiltshire} relations and the station-keeping they
imply.
\end{quote}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Bounded relative motion in the rotating frame traces the two-to-one along-track to radial ellipse of the Clohessy-Wiltshire equations.}]{schematics/sch_relative_motion.png}}
\caption{Bounded relative motion in the rotating frame traces the
two-to-one along-track to radial ellipse of the Clohessy-Wiltshire
equations.}
\end{figure}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Starting from the rotating-frame acceleration (28.2) with
  \(\mathbf{n} = n\,\hat{\mathbf{z}}\), show that expanding the radial
  gravitational term \(-\mu/(r_0+x)^2\) to first order in \(x\) gives
  \(-n^2 r_0 + 2 n^2 x\), and that the constant part cancels against the
  centrifugal term acting on the reference radius. Identify which term
  in (28.1) the surviving \(2 n^2 x\) contribution corresponds to, and
  confirm that the cross-track equation \(\ddot z + n^2 z = a_z\)
  describes simple harmonic motion at the orbital rate \(n\).
\item
  Using the secular along-track growth (28.3), estimate the time at
  which the along-track offset reaches the 150-m spacing budget for a
  differential drag acceleration
  \(\delta a_{\mathrm{drag}} = 1\times 10^{-7}\ \mathrm{m\,s^{-2}}\),
  following the inversion in (28.4). Then repeat the estimate for the
  case in which differential drag is ten times larger,
  \(\delta a_{\mathrm{drag}} = 1\times 10^{-6}\ \mathrm{m\,s^{-2}}\),
  and report the new budget time. {[}expect \(t_{\mathrm{budget}}\) of
  order \(10^4\) s in the first case{]}
\item
  The chapter states that differential atmospheric drag varies by a
  factor of one hundred over the solar cycle. Using the quadratic
  scaling of (28.3) and the inversion of (28.4), determine by what
  factor the budget time \(t_{\mathrm{budget}}\) changes when
  \(\delta a_{\mathrm{drag}}\) increases by a factor of one hundred from
  solar minimum to solar maximum. State whether the formation must close
  its relative-navigation and thrust loop more or less frequently at
  solar maximum, and explain in one sentence why this sets the maneuver
  cadence rather than the total propellant.
\item
  Size the maximum tolerable constant differential along-track
  acceleration \(\delta a\) such that the along-track offset (28.3)
  remains below the 150-m spacing budget over a control interval of one
  orbital period \(T = 2\pi/n\). Express your answer in terms of
  \(\Delta y_{\max}\) and \(n\), and then evaluate it for a reference
  orbit with mean motion
  \(n = 1.1\times 10^{-3}\ \mathrm{rad\,s^{-1}}\).
\end{enumerate}
...
\textbf{Albedo.} The fraction of incident sunlight an illuminated body
reflects; Earth's mean value of about 0.30 sets the reflected-solar load
on a spacecraft surface.

\textbf{Beta angle.} The angle between the orbital plane and the Sun
line; a large beta keeps the orbit out of Earth's shadow.

\textbf{Dawn-dusk orbit.} A sun-synchronous orbit whose plane lies near
the day-night terminator, giving near-continuous sunlight.

\textbf{De-orbit.} The controlled lowering of an orbit at end of life so
the spacecraft re-enters within the regulatory window.

\textbf{Emissivity.} The efficiency, between zero and one, with which a
surface radiates thermal energy relative to a black body.

\textbf{Free-space path loss.} The geometric fall-off of received signal
power with the square of range.

\textbf{Ground track.} The locus of sub-satellite points traced on
Earth's surface over an orbit.

\textbf{Hohmann transfer.} The two-burn maneuver between two coplanar
circular orbits that minimizes the velocity change.

\textbf{Junction temperature.} The temperature of the active silicon in
a processor, bounded by an allowable maximum.

\textbf{k-of-n redundancy.} A system that functions when at least k of n
identical units survive.

\textbf{Levelized cost of compute.} The discounted lifetime cost divided
by the useful computation delivered, the orbital analogue of a levelized
cost of energy.

\textbf{Link budget.} The accounting of gains and losses that determines
whether an optical or radio link closes at a given range and data rate.

\textbf{Model-FLOPs utilization.} The fraction of a processor's peak
floating-point capability actually used by a workload.

\textbf{Nodal precession.} The drift of the orbital plane's ascending
node caused by Earth's oblateness.

\textbf{Single-event upset.} A transient bit flip in memory or logic
caused by a single energetic particle.

\textbf{Station-keeping.} The propulsive effort that holds a satellite
in its assigned relative position.

\textbf{Sun-synchronous orbit.} An orbit whose nodal precession matches
Earth's mean motion about the Sun, fixing the local solar time of the
ground track.

\textbf{Total ionizing dose.} The cumulative ionizing energy deposited
in a material over the mission, the slow-accumulation part of the
radiation environment.

\textbf{View factor.} The fraction of a surface's hemispherical field of
view occupied by another body, here Earth, setting the radiative
coupling between them.

\textbf{Vis-viva.} The energy relation giving orbital speed as a
function of radius and semi-major axis.
